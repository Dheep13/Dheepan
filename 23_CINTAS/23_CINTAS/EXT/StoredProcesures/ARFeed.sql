CREATE OR REPLACE PROCEDURE EXT.CTAS_SP_ARFEED(IN FILENAME varchar(120))
...
DECLARE EXIT HANDLER FOR SQLEXCEPTION 
    ...
    rollback;
--log('Failed');
    update ext.ctas_arbalance_history
set processflag = 1;
...
/*Load into history*/
insert into ext.ctas_arbalance_history(InvoiceNumber ,
ServiceOrderNumber ,
ServiceOrderDate,
DueDate ,
CustomerId ,
...
InvoiceNumber ,
ServiceOrderNumber ,
to_date(ServiceOrderDate, 'MM/DD/YYYY'),
to_date(DueDate, 'MM/DD/YYYY'),
CustomerId ,
BalanceValue,
:v_currInBatchName,
0 from ext.ctas_arbalance_in);
...
where inboundbatch in (
        ...
        from ext.ctas_arbalance_history
        where processflag = 0
    );
if :v_count = 1 then
...
insert into ext.ctas_arbalance_prestage(
        ...
        from cs_salestransaction st
            ...
            inner join ext.ctas_arbalance_history ar on ar.ServiceOrderNumber = so.orderid
...
--this is to delete positions that are not needed
 ...
 delete from ext.ctas_arbalance_prestage ap_out where not exists (
 select 1
 from cs_title ti
...
select count(1) into v_prestageCount from ext.ctas_arbalance_prestage where inboundbatch= :v_currInBatchName;
ext.ctas_event_log (v_proc_name,'Count of records loaded into prestage for inbound file: '||:v_currInBatchName,:v_prestageCount);
 
update ext.ctas_arbalance_prestage
