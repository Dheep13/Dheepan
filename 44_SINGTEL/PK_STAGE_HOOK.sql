...
CREATE OR REPLACE LIBRARY EXT.PK_STAGE_HOOK DEFAULT SCHEMA EXT AS
BEGIN
...
/* start Deepan : commenting out ComInitialpartition procedure since partitioning is not required*/
/*
PUBLIC PROCEDURE ComInitialpartition
(
    IN i_componentValue VARCHAR(255),   
...
BEGIN
    ...
    DECLARE v_partitionname VARCHAR(100) = 'SH_INITIAL_' || IFNULL(:I_Component,'');  
    DECLARE V_subPartitionname VARCHAR(100) = UPPER('Sh_' || IFNULL(:I_Component,'') || '_' ||
        IFNULL(TO_VARCHAR(:I_Periodseq),'')); 
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        ...
        WHERE
            ...
            AND UPPER(TABLE_NAME) = 'SH_QUERY_RESULT'  
...
END;
*/
/* end Deepan : commenting out ComInitialpartition procedure since partitioning is not required*/
