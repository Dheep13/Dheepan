...
-- CALL SQLSCRIPT_PRINT:PRINT_LINE( 'Creating library ''EXT.PKG_REPORTING_EXTRACT_R2_2''...' );
-- go

CREATE OR REPLACE LIBRARY EXT.PKG_REPORTING_EXTRACT_R2_2 LANGUAGE SQLSCRIPT AS
BEGIN
...
/*Deepan : Commenting out  PUBLIC PROCEDURE dbmtk_init_session_global.  PUBLIC PROCEDURE init_session_global() declared in library*/
...
--     IF SESSION_CONTEXT('DBMTK_PKG_INIT_EXT_DBMTK_PKG_REPORTING_EXTRACT_R2_2__DBMTK_INIT_SESSION_GLOBAL') IS NOT NULL
--     THEN
--         RETURN;
--     END IF;

--     /* mark package as initialized */
--     SET SESSION 'DBMTK_PKG_INIT_EXT_DBMTK_PKG_REPORTING_EXTRACT_R2_2__DBMTK_INIT_SESSION_GLOBAL' = :init_timestamp;
...
--         /* Next lines imported from package header (EXT.PKG_REPORTING_EXTRACT_R2_2.PACKAGE.plsql): */

--         vProcName = NULL;

--         vSQLerrm = NULL;

--         vCurYrStartDate = NULL;
...
--         vSTSRoadShowCategory = 'Roadshow';

--         --Eventtype

--         veventtypeid_ccomobile = 'Mobile Closed';

--         veventtypeid_ccotv = 'TV Closed';

--         --Credit Type

--         vcredittypeid_PayAdj = 'Payment Adjustment';

--         vcredittypeid_CEAdj = 'Customer Experience';

--         vcredittypeid_Mobile = 'CCO Mobile VAS';

--         vcredittypeid_TV = 'CCO TV';

--         vcredittypeid_HandFee = 'TVReconHandlingFee - CCO';

--         --Operational Compliance

--         vOperational_Compliance = 'Operational Compliance';
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_setVariables' ********************
/* ORIGSQL: PROCEDURE prc_setVariables(vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN BIGINT) IS BEGIN */
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_DeleteTable' ********************
/* ORIGSQL: PROCEDURE prc_DeleteTable(pTableName VARCHAR2,pProcessingUnitSeq IN BIGINT) IS BEGIN */
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_TruncateTablePartition' ********************
/* ORIGSQL: PROCEDURE prc_TruncateTablePartition(pTableName VARCHAR2,vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN CS_PERIODCALENDAR(...) */
...
--********** Creating function 'EXT.PKG_REPORTING_EXTRACT_R2_2:fnc_PipelineWasRun' ********************
/* ORIGSQL: FUNCTION fnc_PipelineWasRun(vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN BIGINT) RETURN BOO(...) */
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_AnalyzeTable' ********************
/* ORIGSQL: PROCEDURE prc_AnalyzeTable(pTableName VARCHAR2) IS BEGIN */
PUBLIC PROCEDURE prc_AnalyzeTable
(
    IN pTableName VARCHAR(255)   /* RESOLVE: Manual edits required: VARCHAR2 parameter(no length): user-configured length=255; adjust as needed */
                                 /* ORIGSQL: pTableName VARCHAR2 */
)
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AnalyzeTable','START','Analyse: '||pTableName,NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AnalyzeTable', 'START', 'Analyse: '||IFNULL(:pTableName,''), NULL);

    /* ORIGSQL: DBMS_STATS.gather_table_stats(ownname => '',tabname => pTableName,estimate_percent => DBMS_STATS.auto_sample_size) */
    -- EXECUTE IMMEDIATE 'CREATE STATISTICS ON '|| ''|| '.'|| IFNULL(:pTableName,'');--Deepan : Commenting out code related to partitions
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AnalyzeTable','END','Analyse Done: '||pTableName,NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AnalyzeTable', 'END', 'Analyse Done: '||IFNULL(:pTableName,''), NULL);
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_AnalyzeTableSubpartition' ********************
/* ORIGSQL: PROCEDURE prc_AnalyzeTableSubpartition(pExtUser VARCHAR2, pRptTableName VARCHAR2, pSubPartitionName VARCHAR2) IS BEGIN */
PUBLIC PROCEDURE prc_AnalyzeTableSubpartition
(
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AnalyzeTableSubpartition','START','Analyse: '||pTableName,NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AnalyzeTableSubpartition', 'START', 'Analyse: '||IFNULL(:pRptTableName,''), NULL);
    ...
    -- EXECUTE IMMEDIATE 'CREATE STATISTICS ON '|| IFNULL(:pExtUser,'') || '.'|| IFNULL(:pRptTableName,'');prc_AnalyzeTableSubpartition
    /* RESOLVE: Review semantics in source vs. target DBMS: Verify conversion from DBMS_STATS.GATHER_TABLE_STATS() to CREATE STATISTICS */

    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AnalyzeTableSubpartition','END','Analyse Done: '||pTableName,NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AnalyzeTableSubpartition', 'END', 'Analyse Done: '||IFNULL(:pRptTableName,''), NULL);
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_AddTableSubpartition' ********************
/* ORIGSQL: PROCEDURE prc_AddTableSubpartition(vExtUser VARCHAR2, vTCTemplateTable VARCHAR2, vTCSchemaName VARCHAR2, vTenantId VARCHAR2, vProcessingUnitSeq IN BIGINT, vPeriodSeq (...) */
PUBLIC PROCEDURE prc_AddTableSubpartition
(
...
BEGIN
 ...
 /*Deepan : Partition not required, hence commenting out both cursors dbmtk_cursor_968 and c_partitioncheck  */
    ...
    -- DECLARE CURSOR dbmtk_cursor_1877
    -- FOR
    ...
    -- /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AddTableSubpartition','START','Auto create subpartitions',NULL) */
    -- CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AddTableSubpartition', 'START', 'Auto create subpartitions', NULL);

    -- FOR row AS dbmtk_cursor_1877
    -- DO
    ...
    --         /* ORIGSQL: prc_logevent(:vPeriodName,'prc_AddTableSubpartition','START','Auto create partition',v_partitionname) */
    --         CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_AddTableSubpartition', 'START', 'Auto create partition', :v_partitionname);
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_TruncateTableSubpartition' ********************
/* ORIGSQL: PROCEDURE prc_TruncateTableSubpartition(vRptTableName VARCHAR2, vSubpartitionName VARCHAR2) IS BEGIN */
PUBLIC PROCEDURE prc_TruncateTableSubpartition
(
    IN vRptTableName VARCHAR(255),   /* RESOLVE: Manual edits required: VARCHAR2 parameter(no length): user-configured length=255; adjust as needed */
                                     /* ORIGSQL: vRptTableName VARCHAR2 */
...
BEGIN
   ...
   DECLARE vCalendarName VARCHAR(100);
    ...
    --- 30 Nov 2017 Tharanikumar  Initial release
    ****************************************************************************************************************/
    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_TruncateTableSubpartition','START','Start truncate table '||vSubPartitionName,NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_TruncateTableSubpartition', 'START', 'Start truncate table '||IFNULL(:vSubpartitionName,''), NULL);

    /* RESOLVE: Dynamically generated SQL: Dynamically generated SQL, as executed by Execute-Immediate, convert manually */
    /* ORIGSQL: EXECUTE IMMEDIATE 'ALTER TABLE ' ||vRptTableName || ' TRUNCATE SUBPARTITION ' || vSubPartitionName || ' DROP STORAGE' ; */
    EXECUTE IMMEDIATE 'ALTER TABLE ' ||IFNULL(:vRptTableName,'') || ' TRUNCATE SUBPARTITION ' || IFNULL(:vSubpartitionName,'') || ' DROP STORAGE';
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_buildbasetables' ********************
/* ORIGSQL: PROCEDURE prc_buildbasetables(vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN BIGINT) IS Curso(...) */
...
BEGIN SEQUENTIAL EXECUTION
 ...
 DECLARE vProcessingUnitSeq BIGINT;
        ...
        WHERE
            ...
            (
                ...
                FROM
                    cs_pareporting
                WHERE
                    effectivestartdate < :vPeriodEndDate
    ...
    -- DECLARE vPaReporting EXT.PKG_REPORTING_EXTRACT_R2_2__PRC_BUILDBASETABLES__HierType;  /* ORIGSQL: vPaReporting HierType; */
    ...
    ----- Converted type 'HierType' to 'EXT.PKG_REPORTING_EXTRACT_R2_2__PRC_BUILDBASETABLES__HierType'
    TYPE HierType IS TABLE OF EXT.RPT_BASE_PAREPORTING%ROWTYPE INDEX BY PLS_INTEGER;
    ...
    -- CALL prc_AnalyzeTable('rpt_base_padimension'); --Deepan : Commenting out code related to partitions
    ...
    -- CALL prc_AnalyzeTable('rpt_base_salestransaction');--Deepan : Commenting out code related to partitions
    ...
    -- CALL prc_AnalyzeTable('rpt_base_pareporting');--Deepan : Commenting out code related to partitions
  ...
  /*Deepan : replaced the entire logic without using cursors in the above insert*/
    ...
    /* ORIGSQL: prc_AnalyzeTable('rpt_base_deposit') */
    -- CALL prc_AnalyzeTable('rpt_base_deposit');--Deepan : Commenting out code related to partitions

    /* ORIGSQL: prc_AnalyzeTable('rpt_base_incentive') */
    -- CALL prc_AnalyzeTable('rpt_base_incentive');
    ...
    /* ORIGSQL: prc_AnalyzeTable('rpt_base_incentive') */
    -- CALL prc_AnalyzeTable('rpt_base_incentive');--Deepan : Commenting out code related to partitions

    /* ORIGSQL: prc_AnalyzeTable('rpt_base_measurement') */
    -- CALL prc_AnalyzeTable('rpt_base_measurement');--Deepan : Commenting out code related to partitions
    ...
    -- CALL prc_AnalyzeTable('rpt_base_credit');--Deepan : Commenting out code related to partitions
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_buildreportingtables' ********************
/* ORIGSQL: PROCEDURE prc_buildreportingtables(vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN BIGINT,prep(...) */
...
BEGIN
  /* sapdbmtk: one or more DECLARE statements below were moved upwards, before the first executable statement */ 
    ...
    DECLARE v_reportlist VARCHAR(30000); 
    ...
    FOR
        ...
        WHERE
            ...
            AND Pos.RemoveDate = EXT.PKG_REPORTING_EXTRACT_R2:cEndofTime  /* ORIGSQL: PKG_REPORTING_EXTRACT_R2.cEndofTime */
            --And Pos.PositionGroupSeq  = Pgp.PositionGroupSeq   --sudhir
            AND Pgp.RemoveDate = EXT.PKG_REPORTING_EXTRACT_R2:cEndofTime  /* ORIGSQL: PKG_REPORTING_EXTRACT_R2.cEndofTime */
            AND per.periodseq = :vPeriodSeq
            AND per.calendarseq = :vCalendarSeq
            AND per.removedate = EXT.PKG_REPORTING_EXTRACT_R2:cEndofTime  /* ORIGSQL: PKG_REPORTING_EXTRACT_R2.cEndofTime */
    ...
    ****************************************************************************************************************/
    ...
    -- :vProcessingUnitSeqext BIGINT;;/* NOT CONVERTED! */
    -- :vpipelinerundate       cs_credit.pipelinerundate%TYPE;;/* NOT CONVERTED! */  /* RESOLVE: Identifier not found: Table/Column 'cs_credit.pipelinerundate' not found (for %TYPE declaration) */
    -- :vrunmode               cs_pipelinerun.runmode%TYPE;;/* NOT CONVERTED! */  /* RESOLVE: Identifier not found: Table/Column 'cs_pipelinerun.runmode' not found (for %TYPE declaration) */
    -- :vpipelinerunseq        cs_pipelinerun.pipelinerunseq%TYPE;;/* NOT CONVERTED! */
  DECLARE vPeriodName VARCHAR(100);
  DECLARE vProcessingUnitSeq BIGINT;
  DECLARE vCalendarName VARCHAR(100);
  DECLARE vPeriodShortName VARCHAR(100);
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Params : Period:'||vPeriodSeq||',Processing: ' ||vProcessingUnitSeq||', CalendarSeq :' ||vCalendarSeq||', Report gro(...) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Params : Period:'||IFNULL(:vPeriodSeq,'')||',Processing: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', CalendarSeq :'||IFNULL(:vCalendarSeq,'')||', Report group : '|| IFNULL(:preportgroup,''), NULL); /*Deepan: SQL_ERROR_MESSAGE Cannot be used here replacing with NULL*/
    ...
    --to be --
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Params : vpipelinerundate:'||vpipelinerundate||',vrunmode: ' ||vrunmode||', vpipelinerunseq :' ||vpipelinerunseq,sql(...) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Params : vpipelinerundate:'||IFNULL(:vpipelinerundate,'')||',vrunmode: '||IFNULL(:vrunmode,'')||', vpipelinerunseq :'||IFNULL(:vpipelinerunseq,''), NULL); --/*Deepan: Cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

    --prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started Full Mode',sqlerrm);
    BEGIN 
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Enter Build Reporting Table BEGIN',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Enter Build Reporting Table BEGIN', NULL 
    ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'v_reportlist :' ||v_reportlist,sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'v_reportlist :'||IFNULL(:v_reportlist,''), NULL 
    ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
    BEGIN
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Executing truncate table stel_classifier_Tab',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Executing truncate table stel_classifier_Tab', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Inserting into stel_classifier.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Inserting into stel_classifier.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END;
    --to be added
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'v_reportlist :' ||v_reportlist,sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'v_reportlist :'||IFNULL(:v_reportlist,''), NULL 
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START-BSC Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-BSC Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Start-PRC_BSC_PAY_MTHQTR_ACHV -'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||'(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-PRC_BSC_PAY_MTHQTR_ACHV -'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'End-PRC_BSC_PAY_MTHQTR_ACHV',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-PRC_BSC_PAY_MTHQTR_ACHV', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Start-PRC_BSC_ADV_COMM_PAY'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||',vCal(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-PRC_BSC_ADV_COMM_PAY'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'End-PRC_BSC_ADV_COMM_PAY',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-PRC_BSC_ADV_COMM_PAY', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        --to be added
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,':vPeriodShortname : '||:vPeriodShortname,sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, ':vPeriodShortname : '||IFNULL(:vPeriodShortname,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        IF :vPeriodShortname IN ('Mar', 'Jun', 'Sep', 'Dec')
        THEN
            -- Quaterly Report Generation only

            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START - PRC_BSC_QTR_PAY_SUM -QTR- Started'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - PRC_BSC_QTR_PAY_SUM -QTR- Started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END - PRC_BSC_QTR_PAY_SUM -QTR- Ended',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - PRC_BSC_QTR_PAY_SUM -QTR- Ended', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */

            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START - PRC_BSC_QTR_HIGHLIGHT -QTR- started'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessi(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - PRC_BSC_QTR_HIGHLIGHT -QTR- started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END - PRC_BSC_QTR_HIGHLIGHT -QTR- Ended',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - PRC_BSC_QTR_HIGHLIGHT -QTR- Ended', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'End-BSC Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-BSC Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Start-CCO Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-CCO Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_CCO_MOBILE_PAYOUTSUMM - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUni(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_MOBILE_PAYOUTSUMM - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_CCO_MOBILE_PAYOUTSUMM - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_MOBILE_PAYOUTSUMM - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_CCO_MOBILE_RAWDATA - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSe(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_MOBILE_RAWDATA - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_CCO_MOBILE_RAWDATA - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_MOBILE_RAWDATA - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_CCO_SINGTELTV_DETAILAI - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_SINGTELTV_DETAILAI - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_CCO_SINGTELTV_DETAILAI - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_SINGTELTV_DETAILAI - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSe(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END-CCO Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END-CCO Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Start-STS Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-STS Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessi(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vP(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_STSRCSDS_PAYEE_SUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_STSRCSDS_PAYEE_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_STSRCSDS_PAYEE_SUMMARY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_STSRCSDS_PAYEE_SUMMARY - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_STS_ROADSHOW -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||',(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_ROADSHOW -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        -- It should be always after PRC_STS_PAYEE_INDIVIDUAL
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_STS_ROADSHOW -  ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_ROADSHOW -  ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Started - PRC_STS_COMM_HIGHLIGHT -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitS(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_COMM_HIGHLIGHT -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        -- It should be always after PRC_STS_PAYEE_INDIVIDUAL
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended - PRC_STS_COMM_HIGHLIGHT -  ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_COMM_HIGHLIGHT -  ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END-STS Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END-STS Report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START-RCS report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-RCS report Group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vPro(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_RCS_MICHAEL_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_RCS_MICHAEL_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_RCS_MICHAEL_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_RCS_MICHAEL_INDIVIDUAL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'End-RCS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-RCS report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START-DS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-DS report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProc(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END -DS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END -DS report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    /*
    --TEPL Report only for Admin Group not for User Group
    if v_reportlist like '%_TEPL_%' THEN
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENTER _TEPL_ report group.',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_AI_SUMMARY - STARTED',sqlerrm);
      PRC_TEPL_AI_SUMMARY('TEPLAISUMMARY',vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_AI_SUMMARY - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MSF_SUMMONTHLY - STARTED',sqlerrm);
      PRC_TEPL_MSF_SUMMONTHLY(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MSF_SUMMONTHLY - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_AI_MSF_DETAIL - STARTED',sqlerrm);
      PRC_TEPL_AI_MSF_DETAIL('TEPLAIMSFDETAIL',vPeriodSeq,vProcessingUnitSeq,vCalendarSeq); -- It should be always after PRC_TEPL_AI_SUMMARY
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_AI_MSF_DETAIL - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MSF_SUMMDETAIL - STARTED',sqlerrm);
      PRC_TEPL_MSF_SUMMDETAIL(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MSF_SUMMDETAIL - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MOBILE_MBB_SPICE - STARTED',sqlerrm);
      PRC_TEPL_MOBILE_MBB_SPICE(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_MOBILE_MBB_SPICE - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_ACT_ACR - STARTED',sqlerrm);
      PRC_TEPL_ACT_ACR('TEPLACTUALACCRUAL',vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_ACT_ACR - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_VOUCHER_COVERNOTE - STARTED',sqlerrm);
      PRC_TEPL_VOUCHER_COVERNOTE(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_VOUCHER_COVERNOTE - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_CONSUMER_COVERNOTEYKM - STARTED',sqlerrm);
      PRC_TEPL_CONSUMER_COVERNOTEYKM('TEPLCVRNOTEKYM',vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_CONSUMER_COVERNOTEYKM - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_CONSUMER_REQMEMO - STARTED',sqlerrm);
      PRC_TEPL_CONSUMER_REQMEMO('TEPLCONSREQMEMO',vPeriodSeq,vProcessingUnitSeq,vCalendarSeq);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Procedure PRC_TEPL_CONSUMER_REQMEMO - ENDED',sqlerrm);
        prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Exit _TEPL_ report group.',sqlerrm);
    End if;
    */
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START - CSTI report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - CSTI report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_CSTI_TRANS_DETAIL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_CSTI_TRANS_DETAIL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_CSTI_TRANS_DETAIL ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_CSTI_TRANS_DETAIL ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END - CSTI report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - CSTI report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START - COMM report group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - COMM report group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_COMM_PAYOUT_MONTHLY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_PAYOUT_MONTHLY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_COMM_PAYOUT_MONTHLY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_PAYOUT_MONTHLY - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_COMM_SAA_SUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||'(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_SAA_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_COMM_SAA_SUMMARY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_SAA_SUMMARY - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_COMM_SAA_SUMMARY - QTR - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_SAA_SUMMARY - QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_COMM_SAA_SUMMARY - QTR - ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_SAA_SUMMARY - QTR - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END - COMM report group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - COMM report group', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START DirectSales report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START DirectSales report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_DIRECTSALES_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_DIRECTSALES_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_DIRECTSALES_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_DIRECTSALES_INDIVIDUAL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        IF :vPeriodShortname IN ('Mar', 'Jun', 'Sep', 'Dec')
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED PRC_DIRECTSALES_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_DIRECTSALES_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED PRC_DIRECTSALES_INDIVIDUAL -QTR- ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_DIRECTSALES_INDIVIDUAL -QTR- ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END DirectSales report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END DirectSales report group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'START - IP_Individual Report Group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - IP_Individual Report Group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAID_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUni(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAID_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAID_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAID_INDIVIDUAL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL - ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProces(...) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- ', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
        END IF;
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END IP_Individual Report Group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END IP_Individual Report Group.', NULL ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/  /* ORIGSQL: sqlerrm */
    END IF;

    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'Ended Full Mode',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended Full Mode', NULL 
    ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */

    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,vCalendarRow.name,'END Build Reporting Table',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END Build Reporting Table', NULL 
    ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2_2:prc_driver' ********************
/* ORIGSQL: PROCEDURE prc_driver(vPeriodSeq IN BIGINT,vProcessingUnitSeq IN BIGINT,vCalendarSeq IN BIGINT,preportgroup IN VA(...) */
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-PRC-DRIVER','vPeriodSeq:'||vPeriodSeq||',vProcessingUnitSeq: ' ||vProcessingUnitSeq||', vCalendarSeq :' ||vCalendarSeq||',prep(...) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-PRC-DRIVER', 'vPeriodSeq:'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', vCalendarSeq :'||IFNULL(:vCalendarSeq,'')||',preportgroup :'||IFNULL(:preportgroup,''), NULL 
    ); /*Deepan : SQL_ERROR_MESSAGE  cannot be used here, replacing with NULL*/ /* ORIGSQL: sqlerrm */

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-prc_setVariables','Setting up variables...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-prc_setVariables', 'Setting up variables...', NULL);

    /* ORIGSQL: prc_setVariables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq) */
    CALL prc_setVariables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-prc_setVariables','Setting up variables...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-prc_setVariables', 'Setting up variables...', NULL);

    --IF odsutils.istorunodsprocs (vprocessingunitseq, vperiodseq)
    --THEN
    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_salestransaction','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_salestransaction', '', NULL);

    --Delete Base Tables here.
    /* ORIGSQL: prc_DeleteTable('rpt_base_salestransaction',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_salestransaction', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_salestransaction','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_salestransaction', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_padimension','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_padimension', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_padimension',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_padimension', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_padimension','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_padimension', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_pareporting','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_pareporting', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_pareporting',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_pareporting', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_pareporting','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_pareporting', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_credit','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_credit', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_credit',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_credit', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_credit','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_credit', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_measurement','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_measurement', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_measurement',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_measurement', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_measurement','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_measurement', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_incentive','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_incentive', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_incentive',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_incentive', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_incentive','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_incentive', '', NULL);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','START-DELETE -rpt_base_deposit','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-DELETE -rpt_base_deposit', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_deposit',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_deposit', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'PRC-DRIVER','END-DELETE -rpt_base_deposit','',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'END-DELETE -rpt_base_deposit', '', NULL);

    --prc_AddTableSubpartition('STELEXT','CS_CREDIT','TCMP','STEL',vProcessingUnitSeq,vPeriodSeq,'rpt_base_salestransaction');

    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_driver','START-prc_buildbasetables','Base Table Populations STARTED...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_driver', 'START-prc_buildbasetables', 'Base Table Populations STARTED...', NULL);

    /* ORIGSQL: prc_buildbasetables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq) */
    CALL prc_buildbasetables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq);

    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_driver','END-prc_buildbasetables','Base Table Populations COMPLETED...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_driver', 'END-prc_buildbasetables', 'Base Table Populations COMPLETED...', NULL);

    --END IF;

    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_driver','START-prc_buildreportingtables','START Reporting Table Populations...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_driver', 'START-prc_buildreportingtables', 'START Reporting Table Populations...', NULL);

    /* ORIGSQL: prc_buildreportingtables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq,preportgroup) */
    CALL prc_buildreportingtables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq, :preportgroup);

    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_driver','END-prc_buildreportingtables','End Reporting table populations...',NULL) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_driver', 'END-prc_buildreportingtables', 'End Reporting table populations...', NULL);
END;
