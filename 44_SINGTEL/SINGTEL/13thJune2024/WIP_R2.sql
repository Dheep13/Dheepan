...
DECLARE vPeriodRow ROW LIKE CS_PERIOD;
   ...
   select * into vPeriodRow from cs_period where periodseq=:vPeriodSeq and removedate =:cEndofTime;
    ...
    -- CALL EXT.prc_setVariables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq);/*Deepan: Uncomment this once the proc is ready*/
    ...
    EXECUTE IMMEDIATE
    'ALTER TABLE rpt_base_salestransaction
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_setVariables' ********************
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_DeleteTable' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_DeleteTable', 'Delete started', ''||IFNULL(:pTableName,''), NULL);
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_DeleteTable', 'Delete End', ''||IFNULL(:pTableName,''), NULL);
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_TruncateTablePartition' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_TruncateTablePartition', 'START', 'Truncating Table Partitions for : '||IFNULL(:pTableName,''), NULL);
...
--********** Creating function 'EXT.PKG_REPORTING_EXTRACT_R2:fnc_PipelineWasRun' ********************
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_AnalyzeTable' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_AnalyzeTable', 'prc_AnalyzeTable started', ''||IFNULL(:pTableName,''), NULL);

    /* ORIGSQL: DBMS_STATS.gather_table_stats(ownname => '',tabname => pTableName,estimate_percent => DBMS_STATS.auto_sample_size) */
    EXECUTE IMMEDIATE 'CREATE STATISTICS ON '|| ''|| '.'|| IFNULL(:pTableName,'');
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_AnalyzeTable', 'prc_AnalyzeTable ended', ''||IFNULL(:pTableName,''), NULL);
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_AnalyzeTableSubpartition' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_AnalyzeTableSubpartition', 'prc_AnalyzeTableSubpartition started', ''||IFNULL(:pRptTableName,''), NULL);
    ...
    EXECUTE IMMEDIATE 'CREATE STATISTICS ON '|| IFNULL(:pExtUser,'') || '.'|| IFNULL(:pRptTableName,'');
    /* RESOLVE: Review semantics in source vs. target DBMS: Verify conversion from DBMS_STATS.GATHER_TABLE_STATS() to CREATE STATISTICS */
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_AnalyzeTableSubpartition', 'prc_AnalyzeTableSubpartition ended', ''||IFNULL(:pRptTableName,''), NULL);
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_AddTableSubpartition' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_AddTableSubpartition', 'START', 'Auto create subpartitions', NULL);
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_TruncateTableSubpartition' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_TruncateTableSubpartition', 'START', 'Start truncate table '||IFNULL(:vSubpartitionName,''), NULL);
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_buildbasetables' ********************
...
-- 
    ...
    -- CALL prc_AnalyzeTable('rpt_base_pareporting');/*Deepan : Partition Not required*/
      ...
      /*Deepan : replacing the entire logic without using cursors*/
        ...
        WHERE par.processingunitseq = :vProcessingUnitSeq
            AND par.effectivestartdate < :vPeriodRow_enddate
            AND par.effectiveenddate >= :vPeriodRow_startdate
            AND effectiveenddate = (SELECT MAX(effectiveenddate)
                                    FROM cs_pareporting
                                    WHERE effectivestartdate < :vPeriodRow_enddate
                                        AND effectiveenddate >= :vPeriodRow_startdate
                                        AND removedate = :cEndofTime  
                                        AND par.descendantpositionseq = descendantpositionseq
                                        AND par.descendantuserid = descendantuserid
                                        AND par.Ancestorpositionseq = ancestorpositionseq)
    ...
    (SELECT i.*, SYSDATE*/
        ...
        INSERT INTO ext.rpt_base_incentive
            ...
            SELECT   /* ORIGSQL: (select i.TENANTID, i.INCENTIVESEQ, i.NAME, i.PAYEESEQ, i.POSITIONSEQ, i.PERIODSEQ, i.PIPELINERUNSEQ, i.PIPELINERUNDATE, i.PLANSEQ, i.RULESEQ, i.VALUE, i.UNITTYPEFORVALUE, i.ISACTIVE, i.RELEASEDATE, i(...) */
                :i_FIELD_TENANTID,
                :i_FIELD_INCENTIVESEQ,
                :i_FIELD_NAME,
                :i_FIELD_PAYEESEQ,
                :i_FIELD_POSITIONSEQ,
                :i_FIELD_PERIODSEQ,
                :i_FIELD_PIPELINERUNSEQ,
                :i_FIELD_PIPELINERUNDATE,
                :i_FIELD_PLANSEQ,
                :i_FIELD_RULESEQ,
                :i_FIELD_VALUE,
                :i_FIELD_UNITTYPEFORVALUE,
                :i_FIELD_ISACTIVE,
                :i_FIELD_RELEASEDATE,
                :i_FIELD_QUOTA,
                :i_FIELD_UNITTYPEFORQUOTA,
                :i_FIELD_ATTAINMENT,
                :i_FIELD_UNITTYPEFORATTAINMENT,
                :i_FIELD_BUSINESSUNITMAP,
                :i_FIELD_GENERICATTRIBUTE1,
                :i_FIELD_GENERICATTRIBUTE2,
                :i_FIELD_GENERICATTRIBUTE3,
                :i_FIELD_GENERICATTRIBUTE4,
                :i_FIELD_GENERICATTRIBUTE5,
                :i_FIELD_GENERICATTRIBUTE6,
                :i_FIELD_GENERICATTRIBUTE7,
                :i_FIELD_GENERICATTRIBUTE8,
                :i_FIELD_GENERICATTRIBUTE9,
                :i_FIELD_GENERICATTRIBUTE10,
                :i_FIELD_GENERICATTRIBUTE11,
                :i_FIELD_GENERICATTRIBUTE12,
                :i_FIELD_GENERICATTRIBUTE13,
                :i_FIELD_GENERICATTRIBUTE14,
                :i_FIELD_GENERICATTRIBUTE15,
                :i_FIELD_GENERICATTRIBUTE16,
                :i_FIELD_GENERICNUMBER1,
                :i_FIELD_UNITTYPEFORGENERICNUMBER1,
                :i_FIELD_GENERICNUMBER2,
                :i_FIELD_UNITTYPEFORGENERICNUMBER2,
                :i_FIELD_GENERICNUMBER3,
                :i_FIELD_UNITTYPEFORGENERICNUMBER3,
                :i_FIELD_GENERICNUMBER4,
                :i_FIELD_UNITTYPEFORGENERICNUMBER4,
                :i_FIELD_GENERICNUMBER5,
                :i_FIELD_UNITTYPEFORGENERICNUMBER5,
                :i_FIELD_GENERICNUMBER6,
                :i_FIELD_UNITTYPEFORGENERICNUMBER6,
                :i_FIELD_GENERICDATE1,
                :i_FIELD_GENERICDATE2,
                :i_FIELD_GENERICDATE3,
                :i_FIELD_GENERICDATE4,
                :i_FIELD_GENERICDATE5,
                :i_FIELD_GENERICDATE6,
                :i_FIELD_GENERICBOOLEAN1,
                :i_FIELD_GENERICBOOLEAN2,
                :i_FIELD_GENERICBOOLEAN3,
                :i_FIELD_GENERICBOOLEAN4,
                :i_FIELD_GENERICBOOLEAN5,
                :i_FIELD_GENERICBOOLEAN6,
                :i_FIELD_PROCESSINGUNITSEQ,
                :i_FIELD_ISREPORTABLE,
                CURRENT_TIMESTAMP   /* ORIGSQL: sysdate */
            FROM
                cs_incentive i,
                cs_period per
            WHERE
                :i_FIELD_periodseq = per.periodseq
                AND :i_FIELD_processingunitseq = :vProcessingUnitSeq
                --AND per.startdate >= vCurYrStartDate
                --AND per.enddate <= vPeriodRow.enddate
                AND per.removedate = :cEndofTime
                AND (per.periodseq = :vPeriodSeq
                    OR per.shortname IN ('Mar', 'Jun', 'Sep', 'Dec'))
                AND (:i_FIELD_pipelinerunseq IN
                    (
                        SELECT   /* ORIGSQL: (SELECT pr.pipelineRunSeq FROM cs_pipelinerun pr, cs_stagesummary ss, cs_stagetype st WHERE pr.pipelineRunSeq = ss.pipelineRunSeq AND ss.stageTypeSeq = st.stageTypeSeq AND st.name = 'Reward' AND pr.pe(...) */
                            pr.pipelineRunSeq
                        FROM
    ...
    CALL prc_AnalyzeTable('rpt_base_measurement');
    ...
    CALL prc_AnalyzeTable('rpt_base_credit');
        ...
        INSERT INTO ext.rpt_base_credit
            ...
            WHERE
                ((cr.periodseq = :vPeriodSeq
                    ...
                    OR (cr.releasedate = (TO_DATE(ADD_SECONDS(vPeriodRow.enddate,(86400*-1))))))  /* ORIGSQL: vPeriodRow.enddate-1 */
        ;
...
--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_buildreportingtables' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Params : Period:'||IFNULL(:vPeriodSeq,'')||',Processing: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', CalendarSeq :'||IFNULL(:vCalendarSeq,'')||', Report group : '|| IFNULL(:preportgroup,''), ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */
    ...
    --to be --
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Params : vpipelinerundate:'||vpipelinerundate||',vrunmode: ' ||vrunmode||', vpipelinerunseq :' ||vpipelinerunseq,sql(...) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Params : vpipelinerundate:'||IFNULL(:vpipelinerundate,'')||',vrunmode: '||IFNULL(:vrunmode,'')||', vpipelinerunseq :'||IFNULL(:vpipelinerunseq,''), ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */
    ...
    BEGIN 
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Enter Build Reporting Table BEGIN',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Enter Build Reporting Table BEGIN', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    ...
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'v_reportlist :' ||v_reportlist,sqlerrm) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'v_reportlist :'||IFNULL(:v_reportlist,''), ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */
    BEGIN
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Executing truncate table stel_classifier_Tab',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Executing truncate table stel_classifier_Tab', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Inserting into stel_classifier.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Inserting into stel_classifier.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END;
    --to be added
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'v_reportlist :' ||v_reportlist,sqlerrm) */
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START-BSC Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START-BSC Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Start-PRC_BSC_PAY_MTHQTR_ACHV -'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||'(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Start-PRC_BSC_PAY_MTHQTR_ACHV -'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'End-PRC_BSC_PAY_MTHQTR_ACHV',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'End-PRC_BSC_PAY_MTHQTR_ACHV', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Start-PRC_BSC_ADV_COMM_PAY'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||',vCal(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Start-PRC_BSC_ADV_COMM_PAY'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'End-PRC_BSC_ADV_COMM_PAY',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'End-PRC_BSC_ADV_COMM_PAY', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        --to be added
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'vPeriodRow.Shortname : '||vPeriodRow.shortname,sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'vPeriodRow.Shortname : '||IFNULL(vPeriodRow.shortname,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START - PRC_BSC_QTR_PAY_SUM -QTR- Started'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START - PRC_BSC_QTR_PAY_SUM -QTR- Started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END - PRC_BSC_QTR_PAY_SUM -QTR- Ended',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END - PRC_BSC_QTR_PAY_SUM -QTR- Ended', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */

            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START - PRC_BSC_QTR_HIGHLIGHT -QTR- started'||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessi(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START - PRC_BSC_QTR_HIGHLIGHT -QTR- started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END - PRC_BSC_QTR_HIGHLIGHT -QTR- Ended',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END - PRC_BSC_QTR_HIGHLIGHT -QTR- Ended', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'End-BSC Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'End-BSC Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Start-CCO Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Start-CCO Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_CCO_MOBILE_PAYOUTSUMM - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUni(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_CCO_MOBILE_PAYOUTSUMM - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_CCO_MOBILE_PAYOUTSUMM - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_CCO_MOBILE_PAYOUTSUMM - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_CCO_MOBILE_RAWDATA - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSe(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_CCO_MOBILE_RAWDATA - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_CCO_MOBILE_RAWDATA - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_CCO_MOBILE_RAWDATA - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_CCO_SINGTELTV_DETAILAI - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_CCO_SINGTELTV_DETAILAI - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_CCO_SINGTELTV_DETAILAI - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_CCO_SINGTELTV_DETAILAI - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSe(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END-CCO Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END-CCO Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Start-STS Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Start-STS Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessi(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vP(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  ', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_STSRCSDS_PAYEE_SUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_STSRCSDS_PAYEE_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_STSRCSDS_PAYEE_SUMMARY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_STSRCSDS_PAYEE_SUMMARY - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_STS_ROADSHOW -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||',(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_STS_ROADSHOW -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        -- It should be always after PRC_STS_PAYEE_INDIVIDUAL
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_STS_ROADSHOW -  ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_STS_ROADSHOW -  ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Started - PRC_STS_COMM_HIGHLIGHT -  '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitS(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Started - PRC_STS_COMM_HIGHLIGHT -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        -- It should be always after PRC_STS_PAYEE_INDIVIDUAL
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended - PRC_STS_COMM_HIGHLIGHT -  ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended - PRC_STS_COMM_HIGHLIGHT -  ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END-STS Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END-STS Report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START-RCS report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START-RCS report Group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vPro(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - ', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_RCS_MICHAEL_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUn(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_RCS_MICHAEL_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_RCS_MICHAEL_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_RCS_MICHAEL_INDIVIDUAL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'End-RCS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'End-RCS report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START-DS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START-DS report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProc(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END -DS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END -DS report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    /*
    ...
    */
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START - CSTI report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START - CSTI report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_CSTI_TRANS_DETAIL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_CSTI_TRANS_DETAIL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_CSTI_TRANS_DETAIL ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_CSTI_TRANS_DETAIL ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END - CSTI report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END - CSTI report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START - COMM report group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START - COMM report group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_COMM_PAYOUT_MONTHLY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_COMM_PAYOUT_MONTHLY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_COMM_PAYOUT_MONTHLY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_COMM_PAYOUT_MONTHLY - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_COMM_SAA_SUMMARY - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnitSeq||'(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_COMM_SAA_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_COMM_SAA_SUMMARY - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_COMM_SAA_SUMMARY - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_COMM_SAA_SUMMARY - QTR - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_COMM_SAA_SUMMARY - QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_COMM_SAA_SUMMARY - QTR - ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_COMM_SAA_SUMMARY - QTR - ', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END - COMM report group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END - COMM report group', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START DirectSales report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START DirectSales report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_DIRECTSALES_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUnit(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_DIRECTSALES_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_DIRECTSALES_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_DIRECTSALES_INDIVIDUAL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        IF vPeriodRow.shortname IN ('Mar', 'Jun', 'Sep', 'Dec')
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED PRC_DIRECTSALES_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED PRC_DIRECTSALES_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED PRC_DIRECTSALES_INDIVIDUAL -QTR- ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED PRC_DIRECTSALES_INDIVIDUAL -QTR- ', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END DirectSales report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END DirectSales report group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'START - IP_Individual Report Group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'START - IP_Individual Report Group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAID_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessingUni(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_INTPREPAID_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAID_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_INTPREPAID_INDIVIDUAL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL - '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProcessing(...) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL - ', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            -- Quaterly Report Generation only
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :' ||vPeriodSeq||',vProcessingUnitSeq:' || vProces(...) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''), ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
            ...
            /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- ',sqlerrm) */
            CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- ', ::SQL_ERROR_MESSAGE 
            );  /* ORIGSQL: sqlerrm */
        END IF;
        /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END IP_Individual Report Group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END IP_Individual Report Group.', ::SQL_ERROR_MESSAGE 
        );  /* ORIGSQL: sqlerrm */
    END IF;

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'Ended Full Mode',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'Ended Full Mode', ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,v_procname,vCalendarRow.name,'END Build Reporting Table',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, :v_procname, vCalendarRow.name, 'END Build Reporting Table', ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */
END;

--********** Creating procedure 'EXT.PKG_REPORTING_EXTRACT_R2:prc_driver' ********************
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-PRC-DRIVER','vPeriodSeq:'||vPeriodSeq||',vProcessingUnitSeq: ' ||vProcessingUnitSeq||', vCalendarSeq :' ||vCalendarSeq||',prep(...) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-PRC-DRIVER', 'vPeriodSeq:'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', vCalendarSeq :'||IFNULL(:vCalendarSeq,'')||',preportgroup :'||IFNULL(:preportgroup,''), ::SQL_ERROR_MESSAGE 
    );  /* ORIGSQL: sqlerrm */

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-prc_setVariables','Setting up variables...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-prc_setVariables', 'Setting up variables...', NULL);

    /* ORIGSQL: prc_setVariables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq) */
    CALL prc_setVariables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-prc_setVariables','Setting up variables...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-prc_setVariables', 'Setting up variables...', NULL);

    --IF odsutils.istorunodsprocs (vprocessingunitseq, vperiodseq)
    --THEN
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_salestransaction','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_salestransaction', '', NULL);
    ...
    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_salestransaction','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_salestransaction', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_padimension','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_padimension', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_padimension',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_padimension', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_padimension','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_padimension', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_pareporting','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_pareporting', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_pareporting',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_pareporting', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_pareporting','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_pareporting', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_credit','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_credit', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_credit',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_credit', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_credit','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_credit', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_measurement','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_measurement', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_measurement',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_measurement', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_measurement','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_measurement', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_incentive','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_incentive', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_incentive',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_incentive', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_incentive','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_incentive', '', NULL);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','START-DELETE -rpt_base_deposit','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'START-DELETE -rpt_base_deposit', '', NULL);

    /* ORIGSQL: prc_DeleteTable('rpt_base_deposit',vProcessingUnitSeq) */
    CALL prc_DeleteTable('rpt_base_deposit', :vProcessingUnitSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'PRC-DRIVER','END-DELETE -rpt_base_deposit','',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'PRC-DRIVER', 'END-DELETE -rpt_base_deposit', '', NULL);

    --prc_AddTableSubpartition('STELEXT','CS_CREDIT','TCMP','STEL',vProcessingUnitSeq,vPeriodSeq,'rpt_base_salestransaction');

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'prc_driver','START-prc_buildbasetables','Base Table Populations STARTED...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_driver', 'START-prc_buildbasetables', 'Base Table Populations STARTED...', NULL);

    /* ORIGSQL: prc_buildbasetables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq) */
    CALL prc_buildbasetables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'prc_driver','END-prc_buildbasetables','Base Table Populations COMPLETED...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_driver', 'END-prc_buildbasetables', 'Base Table Populations COMPLETED...', NULL);

    --END IF;

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'prc_driver','START-prc_buildreportingtables','START Reporting Table Populations...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_driver', 'START-prc_buildreportingtables', 'START Reporting Table Populations...', NULL);

    /* ORIGSQL: prc_buildreportingtables(vPeriodSeq,vProcessingUnitSeq,vCalendarSeq,preportgroup) */
    CALL prc_buildreportingtables(:vPeriodSeq, :vProcessingUnitSeq, :vCalendarSeq, :preportgroup);

    /* ORIGSQL: prc_logevent(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name,'prc_driver','END-prc_buildreportingtables','End Reporting table populations...',NULL) */
    CALL EXT.PRC_LOGEVENT(PKG_REPORTING_EXTRACT_R2.vPeriodRow.name, 'prc_driver', 'END-prc_buildreportingtables', 'End Reporting table populations...', NULL);
END;

/* ORIGSQL: END PKG_REPORTING_EXTRACT_R2; */
END;/* end of library */
