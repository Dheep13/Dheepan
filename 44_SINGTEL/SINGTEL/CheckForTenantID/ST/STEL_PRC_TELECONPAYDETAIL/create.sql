CREATE PROCEDURE EXT.STEL_PRC_TELECONPAYDETAIL
(
...
BEGIN
    ...
    v_ComponentName = 'STEL_PRC_TELECONPAYDETAIL';
    ...
    -- Deleting  table data 
    /* ORIGSQL: DELETE STEL_RPT_MMP_TELECONPAYDET WHERE PERIODSEQ = IN_PERIODSEQ AND PROCESSINGU(...) */
    DELETE
    FROM
        EXT.STEL_RPT_MMP_TELECONPAYDET
    WHERE
        PERIODSEQ = :IN_PERIODSEQ
    ...
    /* ORIGSQL: STEL_PROC_RPT_partitions (IN_PERIODSEQ, 'STEL_RPT_MMP_TELECONPAYDET') */
    CALL EXT.STEL_PROC_RPT_PARTITIONS(:IN_PERIODSEQ, 'EXT.STEL_RPT_MMP_TELECONPAYDET');

    -- managing table partitions

    -- Below block for  section 1
    /* RESOLVE: Identifier not found: Table/view 'DBMTK_USER_NAME.STEL_RPT_MMP_TELECONPAYDET' not found */

    /* ORIGSQL: INSERT INTO STEL_RPT_MMP_TELECONPAYDET (TENANTID, PERIODSEQ, PERIODNAME, PROCESS(...) */
    ...
    INSERT INTO EXT.STEL_RPT_MMP_TELECONPAYDET
        (
            TENANTID,
            PERIODSEQ,
            PERIODNAME,
        ...
        INNER JOIN
            ...
            AND m.name LIKE 'P%Telecontinent%'
            ...
            SELECT   /* ORIGSQL: (SELECT v_Tenant TENANTID, IN_PERIODSEQ PERIODSEQ, v_PeriodName PERIODNAME, IN_P(...) */
                ...
                WHERE
                    ...
                    (
                        ...
                        WHERE
                            UPPER(par.lastname) LIKE '%TELECONTINENT%' 
