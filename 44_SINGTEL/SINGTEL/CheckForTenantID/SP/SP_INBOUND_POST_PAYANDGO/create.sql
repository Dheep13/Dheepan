CREATE PROCEDURE EXT.SP_INBOUND_POST_PAYANDGO
()
LANGUAGE SQLSCRIPT
...
BEGIN
    ...
    /* ORIGSQL: execute immediate 'truncate table STEL_TEMP_SIMONLY'; */
    /* RESOLVE: Identifier not found: Table/view 'DBMTK_USER_NAME.STEL_TEMP_SIMONLY' not found */

    /* ORIGSQL: truncate table STEL_TEMP_SIMONLY ; */
    EXECUTE IMMEDIATE 'TRUNCATE TABLE EXT.STEL_TEMP_SIMONLY';

    /* ORIGSQL: insert into STEL_TEMP_SIMONLY select bill.custid, bill.contact, st.salestransact(...) */
    INSERT INTO EXT.STEL_TEMP_SIMONLY
        SELECT   /* ORIGSQL: select bill.custid, bill.contact, st.salestransactionseq, ROW_NUMBER() OVER (PAR(...) */
            bill.custid,
            bill.contact,
            st.salestransactionseq,
            ROW_NUMBER() OVER (PARTITION BY bill.custid, bill.contact ORDER BY st.compensationdate) AS rn
            /* --, st.* */
        FROM
        ...
        INNER JOIN
         cs_transactionaddress bill
            ON st.salestransactionseq = bill.salestransactionseq
            AND st.billtoaddressseq = bill.transactionaddressseq
            AND bill.tenantid = 'STEL'  -- and bill.processingunitseq=st.processingunitseq
        ...
        WHERE
            ...
            AND UPPER(st.genericattribute31) LIKE '%SIM%ONLY%'
            AND datasource = 'BCC'
            --AND NVL (st.genericboolean1, 0) <> 1
            AND genericattribute12 <> 'CE'
            AND genericattribute10 <> 'CE'
            AND genericattribute22 = 'AC'
            AND genericattribute9 = 'M';

    --STEP 1
    /*set to reconciled*/ 

    /* RESOLVE: MERGE statement: Additional manual conversion of MERGE INTO may be required */
    /* ORIGSQL: MERGE INTO STEL_DATA_PICK_AND_GO tgt USING (SELECT tmp.contact AS Serv_number, t(...) */
    MERGE INTO EXT.STEL_DATA_PICK_AND_GO AS tgt   
        USING
        (
            SELECT   /* ORIGSQL: (SELECT tmp.contact Serv_number, tmp.custid, st.genericattribute3 DealerCode, st(...) */
                tmp.contact AS Serv_number,
                tmp.custid,
            ...
            FROM
               ...
               cs_eventtype et,
                EXT.STEL_DATA_PICK_AND_GO x,
                EXT.INBOUND_cfg_parameter par,
                EXT.STEL_TEMP_SIMONLY tmp
                /* RESOLVE: Oracle Database link: Remote table/view 'DBMTK_USER_NAME.cs_salestransaction@stelext' at Oracle DBlink 'stelext' is converted to HANA virtual table 'DBMTK_USER_NAME.cs_salestransaction_AT_STELEXT'; see 'dbmtk_create_remote_tables.sqlscript' for creating this virtual table. */
                /* RESOLVE: Oracle Database link: Remote table/view 'DBMTK_USER_NAME.cs_eventtype@stelext' at Oracle DBlink 'stelext' is converted to HANA virtual table 'DBMTK_USER_NAME.cs_eventtype_AT_STELEXT'; see 'dbmtk_create_remote_tables.sqlscript' for creating this virtual table. */
            WHERE
                tmp.salestransactionseq = st.salestransactionseq
                AND tmp.rn = 1
                AND x.SERVICE_NO = tmp.contact -- Service Number
                -- AND x.dealer_code = st.genericattribute3  -- Dealer Code
                AND x.CUSTOMER_NRIC = tmp.custid -- CustomerID
                -- AND X.PRODUCT = st.productid
                AND et.datatypeseq = st.eventtypeseq
                AND UPPER(st.genericattribute31) LIKE '%SIM%ONLY%' 
                AND et.removedate >CURRENT_TIMESTAMP   /* ORIGSQL: sysdate */
                AND UPPER(et.eventtypeid) LIKE '%CLOSED%' -- take only Closed Orders only 
        ...
        ON (src.Serv_number = TGT.SERVICE_NO
            AND src.custid = TGT.CUSTOMER_NRIC
            -- AND src.DealerCode = TGT.DEALER_CODE
            -- AND src.productid = TGT.PRODUCT
            AND 1 = 1 --NVL (tgt.reconciled, 0) = 0
    ...
    /* RESOLVE: MERGE statement: Additional manual conversion of MERGE INTO may be required */
    /* ORIGSQL: MERGE INTO STEL_DATA_PICK_AND_GO tgt USING (SELECT DISTINCT tmp.contact AS Serv_(...) */
    MERGE INTO EXT.STEL_DATA_PICK_AND_GO AS tgt
        /* RESOLVE: Identifier not found: Table/view 'DBMTK_USER_NAME.STEL_DATA_PICK_AND_GO' not found */
        USING
        (
            SELECT   /* ORIGSQL: (SELECT DISTINCT tmp.contact Serv_number, tmp.custid, st.genericattribute3 Deale(...) */
                DISTINCT
                tmp.contact AS Serv_number,
                tmp.custid,
                st.genericattribute3 AS DealerCode,
            ...
            WHERE
                tmp.rn = 1
                AND tmp.salestransactionseq = st.salestransactionseq
                AND x.SERVICE_NO = tmp.contact -- Service Number
                -- AND x.dealer_code = st.genericattribute3  -- Dealer Code
                AND x.CUSTOMER_NRIC = tmp.custid -- CustomerID
                -- AND X.PRODUCT = st.productid
    ...
    CALL EXT.SP_INBOUND_VALIDATOR(:v_inbound_cfg_parameter.file_type, :v_inbound_cfg_parameter.file_name, :v_inbound_cfg_parameter.file_date, 2);
