CREATE PROCEDURE EXT.RPT_TXNINIT
(
...
BEGIN
    ...
    DECLARE CURSOR dbmtk_cursor_13181
    FOR 
    ...
    DO
        v_insert_cls = 'Insert Into '||IFNULL(:i.tgttbl,'') ||' (
            ...
            ,orderid, alternateordernumber, partname, origperiodseq, origperiodname, payperiodseq, payperiodname
        ) ';
        ...
        THEN
            /*v_select_cls :=  '
                  ...
                  select distinct ''' ||p_rpttype|| ''' RPTTYPE , sysdate, st.salestransactionseq, st.salesorderseq, ta.positionname, pos1.ruleelementownerseq
                  , '''||v_tenant||''' ,  '||v_pu ||', '''|| v_puname||'''
                  , pos1.payeeseq, c.creditseq, c.name creditname, st.compensationdate, pd.monthperiodseq1 ,pd.monthenddate1-1 MonthEndDate
                  ,   pd.monthname1, ct.credittypeid
             , st.datasource, st.value txnvalue, st.numberofunits txnunits, c.value creditvalue
                  ...
                  , pd.monthperiodseqtd, pd.monthnametd
                  ,pd.periodseq, pd.periodname
            
                  from cs_Salestransaction st
                  join cs_transactionassignment ta
                  on st.salestransactionseq=ta.salestransactionseq
             and ta.processingunitseq=st.processingunitseq and ta.compensationdate=st.compensationdate
             and st.tenantid=ta.tenantid
                  join cs_salesorder so
                  on so.salesorderseq=st.salesorderseq and so.removedate='||v_eot||'
             and so.tenantid=st.tenantid and so.processingunitseq=st.processingunitseq
                  join stel_monthhierarchy_tbl pd
            on  pd.monthperiodseq1 = '||p_periodseq||' and pd.periodtypename = '''||i.periodtype||'''
            
            
            
                  join cs_position pos1
                  on ta.positionname= pos1.name and pos1.removedate='||v_eot||'
             ...
             and ('||i.titlefilter||')
             and st.compensationdate between t.effectivestartdate and t.effectiveenddate-1
             and st.tenantid=t.tenantid
            
                  left join cs_position pos2
                  on ta.positionname= pos2.name and pos2.removedate='||v_eot||'
             and  pd.monthenddatetd between pos2.effectivestartdate+1 and pos2.effectiveenddate
             and pos2.tenantid=st.tenantid
                  left join cs_Credit c
                  on c.salestransactionseq=st.salestransactionseq
             and c.tenantid=st.tenantid and c.processingunitseq=st.processingunitseq and ('||i.includecredits||'=''1'')
             and c.salesorderseq=st.salesorderseq
                  left join cs_Credittype ct
                  on c.credittypeseq=ct.datatypeseq and ct.removedate>sysdate
            
                  where  st.compensationdate between pd.monthstartdatetd and pd.monthenddatetd-1
            
            and   st.tenantid = '''||v_tenant||'''
            and st.processingunitseq = '||v_pu||'
             and ('||i.payeefilter||')
             and ('||i.txnfilterclause||')'
             ; */

            --kyap query improvement on 20180909 due to performance issue
        --a) remove pos2 position, dont see any need for it
    --b) remove transactionassignment, as only position found on credits should be paid
--c) pos1 position, join to cs_credits via positionseq=ruleelementownerseq

v_select_cls = '

select distinct ''' ||IFNULL(:p_RPTTYPE,'')|| ''' RPTTYPE , sysdate, st.salestransactionseq, st.salesorderseq, pos1.name, pos1.ruleelementownerseq
...
join cs_salesorder so
on so.salesorderseq=st.salesorderseq and so.removedate=' ||IFNULL(:v_eot,'')||'
and so.tenantid=st.tenantid and so.processingunitseq=st.processingunitseq
join stel_monthhierarchy_tbl pd
...
AND C.PERIODSEQ = pd.monthperiodseq1
and c.salesorderseq=st.salesorderseq
...
join cs_position pos1
on c.positionseq= pos1.ruleelementownerseq
and pos1.removedate=' ||IFNULL(:v_eot,'')||'
and st.compensationdate between pos1.effectivestartdate and pos1.effectiveenddate-1
and pos1.tenantid=st.tenantid

join cs_participant par
on par.payeeseq=C.payeeseq and par.removedate>Sysdate
and st.compensationdate between par.effectivestartdate and par.effectiveenddate-1
and par.tenantid=pos1.tenantid
        ...
        BEGIN 
            DECLARE EXIT HANDLER FOR SQLEXCEPTION
                ...
                BEGIN
                    ...
                    /* ORIGSQL: dbms_output.put_line('truncate period 2'); */
                   -- CALL sapdbmtk.sp_dbmtk_buffered_output_writeln('truncate period 2');
                END;
            ...
            /* ORIGSQL: dbms_output.put_line('truncate period 1'); */
            --CALL sapdbmtk.sp_dbmtk_buffered_output_writeln('truncate period 1');

            /* ORIGSQL: exception when others then */
        END;

        /* ORIGSQL: stel_Sp_logger ('Executing SQL for txninit', 'RPT_TXNINIT', NULL, NULL, v_sql) */
        CALL EXT.STEL_SP_LOGGER('Executing SQL for txninit', 'RPT_TXNINIT', NULL, NULL, :v_sql);

        /* RESOLVE: Dynamically generated SQL: Dynamically generated SQL, as executed by Execute-Immediate, convert manually */
        /* ORIGSQL: EXECUTE IMMEDIATE v_sql ; */
        EXECUTE IMMEDIATE :v_sql;

        /* ORIGSQL: dbms_output.put_line('RPT_TXNINIT done.'); */
        --CALL sapdbmtk.sp_dbmtk_buffered_output_writeln('RPT_TXNINIT done.');
