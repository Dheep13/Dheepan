CREATE LIBRARY "EXT"."PKG_REPORTING_EXTRACT_R3" LANGUAGE SQLSCRIPT AS
BEGIN
...
BEGIN
    ...
    */
    ...
    SET SESSION 'GvCalendarName' = :vCalendarName;
...
BEGIN
    ...
    ****************************************************************************************************************/
    /* ORIGSQL: prc_logevent(:vPeriodName,'prc_TruncateTablePartition','START','Truncating Table Partitions for : '||pTableName,NULL) *//*Deepan : Proc not required*/
...
END;
  PUBLIC PROCEDURE prc_AnalyzeTable/*Deepan : partition not required*/
(
    IN pTableName VARCHAR(255)   /* RESOLVE: Manual edits required: VARCHAR2 parameter(no length): user-configured length=255; adjust as needed */
                                 /* ORIGSQL: pTableName VARCHAR2 */
)
...
(
    ...
    IN vPeriodSeq BIGINT,   /* RESOLVE: Identifier not found: Table/Column 'cs_period.periodseq' not found (for %TYPE declaration) */
                                              /* RESOLVE: Datatype unresolved: Datatype (cs_period.periodseq%TYPE) not resolved for parameter 'prc_AddTableSubpartition.vPeriodSeq' */
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    -- DECLARE CURSOR dbmtk_cursor_2781
    -- FOR
    ...
    -- FOR row AS dbmtk_cursor_2781
    -- DO
    --     v_partitionname = sapdbmtk.sp_f_dbmtk_substring(:row.subpartition_name,0,12);  /* ORIGSQL: SUBSTR(row.subpartition_name, 0, 12) */
    ...
    --     EXECUTE IMMEDIATE 'ALTER TABLE ' ||IFNULL(:vRptTableName,'')||
    --     ' MODIFY PARTITION '|| IFNULL(sapdbmtk.sp_f_dbmtk_substring(:row.subpartition_name,0,12),'') ||  /* ORIGSQL: SUBSTR(row.subpartition_name, 0, 12) */
    --     ' ADD SUBPARTITION ' || IFNULL(:row.subpartition_name,'') ||
    --     ' VALUES (' ||IFNULL(:row.high_value,'')||') TABLESPACE TALLYDATA';
    -- END FOR;  /* ORIGSQL: END LOOP; */
END;
  PUBLIC PROCEDURE prc_TruncateTableSubpartition/*Deepan : Partition not required*/
(
    IN vRptTableName VARCHAR(255),   /* RESOLVE: Manual edits required: VARCHAR2 parameter(no length): user-configured length=255; adjust as needed */
                                     /* ORIGSQL: vRptTableName VARCHAR2 */
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    -- CALL EXT.PRC_LOGEVENT(:vPeriodName, 'prc_TruncateTableSubpartition', 'START', 'Start truncate table '||IFNULL(:vSubpartitionName,''), NULL);/*Deepan : Partition not required*/

    /* RESOLVE: Dynamically generated SQL: Dynamically generated SQL, as executed by Execute-Immediate, convert manually */
    /* ORIGSQL: EXECUTE IMMEDIATE 'ALTER TABLE ' ||vRptTableName || ' TRUNCATE SUBPARTITION ' || vSubPartitionName || ' DROP STORAGE' ; */
    -- EXECUTE IMMEDIATE 'ALTER TABLE ' ||IFNULL(:vRptTableName,'') || ' TRUNCATE SUBPARTITION ' || IFNULL(:vSubpartitionName,'') || ' DROP STORAGE';/*Deepan : Partition not required*/
END;
...
BEGIN SEQUENTIAL EXECUTION
    -- DECLARE DBMTK_TMPVAR_STRING_1 VARCHAR(5000); /*sapdbmtk-generated help variable*/ --/*Deepan : variable Not required*/
    -- DECLARE DBMTK_CTV_PROCID INT := sapdbmtk.sp_f_dbmtk_ctv_procid(); /*sapdbmtk-generated help variable*//*Deepan : variable Not required*/
   ...
   /*Deepan : Commenting out the below cursor based logic and using the insert directly*/
    ...
    -- DECLARE vPaReporting EXT.PKG_REPORTING_EXTRACT_R3__PRC_BUILDBASETABLES__HierType;  /* ORIGSQL: vPaReporting HierType; */
  ...
  DECLARE vPeriodEndDate timestamp;
    ...
    ----- Converted type 'HierType' to 'EXT.PKG_REPORTING_EXTRACT_R3__PRC_BUILDBASETABLES__HierType'
    ...
    CALL prc_AnalyzeTable('rpt_base_padimension');
    ...
    CALL prc_AnalyzeTable('rpt_base_salestransaction');
    ...
    CALL prc_AnalyzeTable('rpt_base_pareporting');
...
WHERE
    ...
    -- CALL prc_AnalyzeTable('rpt_base_deposit');/*Deepan :Partitioning not required*/
    ...
    CALL prc_AnalyzeTable('rpt_base_incentive');/*Deepan :Partitioning not required*/
...
BEGIN
    ...
    ****************************************************************************************************************/
    ...
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'Params : Period:'||vPeriodSeq||',Processing: ' ||vProcessingUnitSeq||', CalendarSeq :' ||vCalendarSeq||', Report gro(...) */
     DECLARE vPeriodName VARCHAR(100);
	 DECLARE vProcessingUnitSeq BIGINT;
     DECLARE vCalendarName VARCHAR(100);
     DECLARE vPeriodShortName VARCHAR(100);
     --DECLARE vpipelinerundate timestamp;
     DECLARE vrunmode varchar(100);
     --DECLARE vpipelinerunseq BIGINT;
    ...
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Params : Period:'||IFNULL(:vPeriodSeq,'')||',Processing: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', CalendarSeq :'||IFNULL(:vCalendarSeq,'')||', Report group : '|| IFNULL(:preportgroup,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */
    ...
    --to be --
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'Params : vpipelinerundate:'||vpipelinerundate||',vrunmode: ' ||vrunmode||', vpipelinerunseq :' ||vpipelinerunseq,sql(...) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Params : vpipelinerundate:'||IFNULL(:vpipelinerundate,'')||',vrunmode: '||IFNULL(:vrunmode,'')||', vpipelinerunseq :'||IFNULL(:vpipelinerunseq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */

    --prc_logevent(:vPeriodName,v_procname,:vCalendarName,'Started Full Mode',sqlerrm);
    BEGIN 
        ...
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'Enter Build Reporting Table BEGIN',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Enter Build Reporting Table BEGIN',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    ...
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'v_reportlist :'||IFNULL(:v_reportlist,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */
    BEGIN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Executing truncate table stel_classifier_Tab',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Inserting into stel_classifier.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END;
    --to be added
    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'v_reportlist :' ||v_reportlist,sqlerrm) */
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-BSC Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-PRC_BSC_PAY_MTHQTR_ACHV -'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-PRC_BSC_PAY_MTHQTR_ACHV',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-PRC_BSC_ADV_COMM_PAY'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-PRC_BSC_ADV_COMM_PAY',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        --to be added
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'vPeriodRow.Shortname : '||vPeriodRow.shortname,sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'vPeriodRow.Shortname : '||IFNULL(:vPeriodShortname,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - PRC_BSC_QTR_PAY_SUM -QTR- Started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - PRC_BSC_QTR_PAY_SUM -QTR- Ended',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - PRC_BSC_QTR_HIGHLIGHT -QTR- started'||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - PRC_BSC_QTR_HIGHLIGHT -QTR- Ended',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'End-BSC Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-BSC Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-CCO Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_MOBILE_PAYOUTSUMM - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_MOBILE_PAYOUTSUMM - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_MOBILE_RAWDATA - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_MOBILE_RAWDATA - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_SINGTELTV_DETAILAI - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_SINGTELTV_DETAILAI - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_CCO_TV_PAYOUTSUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END-CCO Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END-CCO Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Start-STS Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_RC_DS_PAYEE_INDIVIDUAL - Qrt -  ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_STSRCSDS_PAYEE_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_STSRCSDS_PAYEE_SUMMARY - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_ROADSHOW -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_ROADSHOW -  ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Started - PRC_STS_COMM_HIGHLIGHT -  '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended - PRC_STS_COMM_HIGHLIGHT -  ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END-STS Report Group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END-STS Report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-RCS report Group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_STS_RC_DS_PAYEE_INDIVIDUAL -QTR - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_RCS_MICHAEL_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_RCS_MICHAEL_INDIVIDUAL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'End-RCS report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'End-RCS report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START-DS report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END -DS report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    /*
    ...
    */
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - CSTI report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'STARTED PRC_CSTI_TRANS_DETAIL - ',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_CSTI_TRANS_DETAIL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_CSTI_TRANS_DETAIL ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END - CSTI report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - CSTI report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - COMM report group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_PAYOUT_MONTHLY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_PAYOUT_MONTHLY - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_SAA_SUMMARY - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_SAA_SUMMARY - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_COMM_SAA_SUMMARY - QTR - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_COMM_SAA_SUMMARY - QTR - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END - COMM report group',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END - COMM report group',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START DirectSales report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_DIRECTSALES_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_DIRECTSALES_INDIVIDUAL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED PRC_DIRECTSALES_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED PRC_DIRECTSALES_INDIVIDUAL -QTR- ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;

        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END DirectSales report group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END DirectSales report group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;
    ...
    THEN
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'START - IP_Individual Report Group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAID_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAID_INDIVIDUAL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL - '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL - ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
        ...
        THEN
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'STARTED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- '||'Parms:vPeriodSeq :'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq:'|| IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||',vCalendarSeq :'|| IFNULL(:vCalendarSeq,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
            ...
            CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'ENDED - PRC_INTPREPAIDSNR_INDIVIDUAL -QTR- ',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
            );  /* ORIGSQL: sqlerrm */
        END IF;
        /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END IP_Individual Report Group.',sqlerrm) */
        CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END IP_Individual Report Group.',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
        );  /* ORIGSQL: sqlerrm */
    END IF;

    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'Ended Full Mode',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'Ended Full Mode',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */

    /* ORIGSQL: prc_logevent(:vPeriodName,v_procname,:vCalendarName,'END Build Reporting Table',sqlerrm) */
    CALL EXT.PRC_LOGEVENT(:vPeriodName, :v_procname, :vCalendarName, 'END Build Reporting Table',  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */
END;
...
BEGIN
  ...
  DECLARE vPeriodRow ROW LIKE CS_PERIOD;
    ...
    CALL EXT.PRC_LOGEVENT(:vPeriodName, 'PRC-DRIVER', 'START-PRC-DRIVER', 'vPeriodSeq:'||IFNULL(:vPeriodSeq,'')||',vProcessingUnitSeq: '||IFNULL(TO_VARCHAR(:vProcessingUnitSeq),'')||', vCalendarSeq :'||IFNULL(:vCalendarSeq,'')||',preportgroup :'||IFNULL(:preportgroup,''),  NULL /*NULL /*::SQL_ERROR_MESSAGE*/ --/*Deepan: Cannot be used here, replacing with NULL*/ 
    );  /* ORIGSQL: sqlerrm */
