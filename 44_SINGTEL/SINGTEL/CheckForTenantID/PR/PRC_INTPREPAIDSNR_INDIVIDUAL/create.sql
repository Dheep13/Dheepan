CREATE PROCEDURE EXT.PRC_INTPREPAIDSNR_INDIVIDUAL
(
    IN vrptname NVARCHAR(4000),   /* RESOLVE: Identifier not found: Table/Column 'rpt_directsales_mapping.reportname' not found (for %TYPE declaration) */
                                                           /* RESOLVE: Datatype unresolved: Datatype (rpt_directsales_mapping.reportname%TYPE) not resolved for parameter 'PRC_INTPREPAIDSNR_INDIVIDUAL.vrptname' */
    ...
    IN vperiodseq BIGINT,   /* RESOLVE: Identifier not found: Table/Column 'cs_period.periodseq' not found (for %TYPE declaration) */
                                              /* RESOLVE: Datatype unresolved: Datatype (cs_period.periodseq%TYPE) not resolved for parameter 'PRC_INTPREPAIDSNR_INDIVIDUAL.vperiodseq' */
    ...
    IN vprocessingunitseq BIGINT,   /* RESOLVE: Identifier not found: Table/Column 'cs_processingunit.processingunitseq' not found (for %TYPE declaration) */
                                                                      /* RESOLVE: Datatype unresolved: Datatype (cs_processingunit.processingunitseq%TYPE) not resolved for parameter 'PRC_INTPREPAIDSNR_INDIVIDUAL.vprocessingunitseq' */
    ...
    IN vcalendarseq BIGINT,   /* RESOLVE: Identifier not found: Table/Column 'cs_period.calendarseq' not found (for %TYPE declaration) */
                                                  /* RESOLVE: Datatype unresolved: Datatype (cs_period.calendarseq%TYPE) not resolved for parameter 'PRC_INTPREPAIDSNR_INDIVIDUAL.vcalendarseq' */
    ...
    IN vPeriodtype NVARCHAR(50)      /* RESOLVE: Identifier not found: Table/Column 'rpt_directsales_mapping.REPORT_FREQUENCY' not found (for %TYPE declaration) */
                                                                      /* RESOLVE: Datatype unresolved: Datatype (rpt_directsales_mapping.REPORT_FREQUENCY%TYPE) not resolved for parameter 'PRC_INTPREPAIDSNR_INDIVIDUAL.vPeriodtype' */
...
BEGIN
    ...
    -------------------------------------------------------------------------------------------------------------------
    --       Maria Monisha  Initial release
    -------------------------------------------------------------------------------------------------------------------
    DECLARE vProcName VARCHAR(30) = UPPER('PRC_INTPREPAIDSNR_INDIVIDUAL');  /* ORIGSQL: vProcName VARCHAR2(30) := UPPER('PRC_INTPREPAIDSNR_INDIVIDUAL') ; */
    ...
    DECLARE vRptTableName VARCHAR(30) = 'RPT_INTPREPAIDSNR_INDIVIDUAL';  /* ORIGSQL: vRptTableName VARCHAR2(30) := 'RPT_INTPREPAIDSNR_INDIVIDUAL'; */
    ...
    /* ORIGSQL: for i in (select distinct product from ext.rpt_directsales_mapping where reportname = vrptname and allgroups = 'INDIVIDUAL PRODUCTS' AND PRODUCT IS NOT NULL and PRODUCT NOT LIKE 'IndividualAch') Loop CALL(...) */
    DECLARE CURSOR dbmtk_cursor_7321
    FOR 
        /* RESOLVE: Identifier not found: Table/view 'EXT.RPT_DIRECTSALES_MAPPING' not found */

        SELECT   /* ORIGSQL: select distinct product from ext.rpt_directsales_mapping where reportname = vrptname and allgroups = 'INDIVIDUAL PRODUCTS' AND PRODUCT IS NOT NULL and PRODUCT NOT LIKE 'IndividualAch'; */
            DISTINCT
            product
        FROM
        ...
        WHERE
            ...
            AND PRODUCT NOT LIKE 'IndividualAch';
    ...
    DECLARE CURSOR dbmtk_cursor_7324
    FOR 
    ...
    /* ORIGSQL: for i in (select distinct product from ext.rpt_directsales_mapping where reportname = vrptname and allgroups = 'TEAM AVERAGE') Loop CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, vProcName, 'Begin TEAM AVERAGE ME(...) */
    DECLARE CURSOR dbmtk_cursor_7327
    FOR 
        SELECT   /* ORIGSQL: select distinct product from ext.rpt_directsales_mapping where reportname = vrptname and allgroups = 'TEAM AVERAGE'; */
            DISTINCT
            product
        FROM
            ext.rpt_directsales_mapping
    ...
    DECLARE CURSOR dbmtk_cursor_7330
    FOR 
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        ...
        */
        ...
        v_reportgroup = 'InternalPrepaidSnr';
        ...
        /* ORIGSQL: DELETE FROM ext.rpt_INTPREPAIDSNR_INDIVIDUAL WHERE reportgroup=v_reportgroup and periodseq=vperiodseq and processingunitseq=vprocessingunitseq and FREQUENCY=vPeriodtype ; */
        DELETE
        FROM
            ext.rpt_INTPREPAIDSNR_INDIVIDUAL
        WHERE
        ...
        DO
            ...
            v_sql = 'INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL
            ...
            v_sql = 'MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using
        ...
        FOR i AS dbmtk_cursor_7324
        DO
            /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin TEAM PRODUCTS MEASUREMENT completed',NULL,i.product) */
            ...
            /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin TEAM PRODUCTS MEASUREMENT completed',NULL,v_sql) */
            CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'Begin TEAM PRODUCTS MEASUREMENT completed', NULL, :v_sql);

            /* ORIGSQL: COMMIT; */
            COMMIT;
        END FOR;  /* ORIGSQL: end loop; */

        FOR i AS dbmtk_cursor_7327
        DO
            /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin TEAM AVERAGE MEASUREMENT',NULL,i.product) */
            ...
            (
                ...
                MAX(case when rmap.rptcolumnname = ''TEAMTOTALPRODUCTS'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMTOTALPRODUCTS', :i.product, :vPeriodtype),'') ||' end) AS TOTALPRODUCTS,
                MAX(case when rmap.rptcolumnname = ''TEAMPRDTARGET'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPRDTARGET', :i.product, :vPeriodtype),'') ||' end) AS PRDTARGET,
                MAX(case when rmap.rptcolumnname = ''TEAMMULTIPLIERPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMMULTIPLIERPER', :i.product, :vPeriodtype),'') ||' end) AS MULTIPLIERPER,
                MAX(case when rmap.rptcolumnname = ''TEAMPOINTSTARGET'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPOINTSTARGET', :i.product, :vPeriodtype),'') ||' end) AS POINTSTARGET,
                MAX(case when rmap.rptcolumnname = ''TEAMPOINTSACTUALS'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPOINTSACTUALS', :i.product, :vPeriodtype),'') ||' end) AS POINTSACTUALS,
                MAX(case when rmap.rptcolumnname = ''TEAMPOINTSACHIEVEDPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPOINTSACHIEVEDPER', :i.product, :vPeriodtype),'') ||' end) AS POINTSACHIEVEDPER
                FROM ext.rpt_BASE_MEASUREMENT CM,rpt_directsales_mapping rmap
            ...
            and pad.periodseq = mes.periodseq
            and pad.frequency=''' ||IFNULL(:vPeriodtype,'')||'''
            and pad.reportgroup = '''||IFNULL(:v_reportgroup,'')||'''';  /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMTOTALPRODUCTS',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPRDTARGET',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPOINTSTARGET',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPOINTSACTUALS',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPOINTSACHIEVEDPER',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMMULTIPLIERPER',i.product,vPeriodtype) */
                                                                         /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMGAPER',i.product,vPeriodtype) */

            /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin TEAM AVERAGE MEASUREMENT completed',NULL,v_sql) */
        ...
        when matched then update set rpt.OTC = qtr.OTC, rpt.ACHIEVEMENTPER=(qtr.ACHIEVEMENTPER*100), rpt.CONNPER=(nvl(qtr.ACHIEVEMENTPER,0)/NULLIF((nvl(qtr.ACHIEVEMENTPER,0)),0)*100),rpt.POINTSPER=0';  /* ORIGSQL: fun_directsales_mapping(vrptname,'OTC','IndividualAch',vPeriodtype) */
        ...
        (select inc.positionseq,
            ...
            inc.processingunitseq,
            inc.periodseq,
            MAX(case when rmap.rptcolumnname = ''TEAMCONNPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMCONNPER', 'TeamAch', :vPeriodtype),'') ||' end) AS TEAMCONNPER,
            MAX(case when rmap.rptcolumnname = ''TEAMPOINTSPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPOINTSPER', 'TeamAch', :vPeriodtype),'') ||' end) AS TEAMPOINTSPER
        ...
        on (rpt.processingunitseq = qtr.processingunitseq
            ...
            and rpt.frequency=''' ||IFNULL(:vPeriodtype,'')||''')
        when matched then update set rpt.CONNPER=(qtr.TEAMCONNPER*100),rpt.POINTSPER=(qtr.TEAMPOINTSPER*100),rpt.ACHIEVEMENTPER=((nvl(qtr.TEAMCONNPER,0) + nvl(qtr.TEAMPOINTSPER,0))*100)';  /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPOINTSPER','TeamAch',vPeriodtype) */
                                                                                                                                                                                                                                                                                             /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMCONNPER','TeamAch',vPeriodtype) */
        ...
        --EARNED commission --
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin Overall Commission incentive',NULL,'Individual') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'Begin Overall Commission incentive', NULL, 'Individual');
        v_sql = 'INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL
        (tenantid,
            positionseq,
            payeeseq,
        ...
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin Team Earned Commission %',NULL,'Percentage') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'Begin Team Earned Commission %', NULL, 'Percentage');
        v_sql = 'MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using
        (select inc.positionseq,
            inc.payeeseq,
            inc.processingunitseq,
            inc.periodseq,
            MAX(case when rmap.rptcolumnname = ''TEAMCONNPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMCONNPER', 'Team', :vPeriodtype),'') ||' end) AS TEAMCONNPER,
            MAX(case when rmap.rptcolumnname = ''TEAMPOINTSPER'' then ' ||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'TEAMPOINTSPER', 'Team', :vPeriodtype),'') ||' end) AS TEAMPOINTSPER
            FROM ext.rpt_base_incentive inc,rpt_directsales_mapping rmap
            WHERE inc.name in rmap.rulename
            AND rmap.reportname= ''' ||IFNULL(:vrptname,'')||'''
            and inc.periodseq = '||IFNULL(:vperiodseq,'')||'
        ...
        on (rpt.processingunitseq = qtr.processingunitseq
            ...
            and rpt.frequency=''' ||IFNULL(:vPeriodtype,'')||''')
        when matched then update set rpt.ACHIEVEMENTPER=((nvl(qtr.TEAMCONNPER,0) + nvl(qtr.TEAMPOINTSPER,0))*100)';  /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMPOINTSPER','Team',vPeriodtype) */
                                                                                                                                                                                                                     /* ORIGSQL: fun_directsales_mapping(vrptname,'TEAMCONNPER','Team',vPeriodtype) */
        ...
        --GAPER AND POINTSACHIEVEDPER for TEAM OVERALL SECTION
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin Team Earned Commission %',NULL,'Percentage') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'Begin Team Earned Commission %', NULL, 'Percentage'); 

        /* RESOLVE: MERGE statement: Additional manual conversion of MERGE INTO may be required */
        /* ORIGSQL: MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using (SELECT ind.positionseq, ind.payeeseq, ind.processingunitseq, ind.periodseq, MAX(ind.GAPER) AS GAPER, MAX(ind.POINTSACHIEVEDPER) AS POINTSACHIEVEDPER (...) */
        MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL AS rpt
            /* RESOLVE: Identifier not found: Table/view 'EXT.RPT_INTPREPAIDSNR_INDIVIDUAL' not found */
            USING
            (
                SELECT   /* ORIGSQL: (select ind.positionseq, ind.payeeseq, ind.processingunitseq, ind.periodseq, MAX(ind.GAPER) GAPER, MAX(ind.POINTSACHIEVEDPER) POINTSACHIEVEDPER FROM ext.rpt_INTPREPAIDSNR_INDIVIDUAL ind WHERE ind.processi(...) */
                    ind.positionseq,
                    ind.payeeseq,
                    ind.processingunitseq,
                    ind.periodseq,
                    MAX(ind.GAPER) AS GAPER,
                    MAX(ind.POINTSACHIEVEDPER) AS POINTSACHIEVEDPER
                FROM
                    ext.rpt_INTPREPAIDSNR_INDIVIDUAL ind
                WHERE
        ...
        WHEN MATCHED THEN
            UPDATE SET rpt.GAPER = qtr.GAPER,rpt.POINTSACHIEVEDPER = qtr.POINTSACHIEVEDPER;

        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'GAPER AND POINTSACHIEVEDPER for TEAM OVERALL SECTION start',NULL,'GAPER AND POINTSACHIEVEDPER') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'GAPER AND POINTSACHIEVEDPER for TEAM OVERALL SECTION start', NULL, 'GAPER AND POINTSACHIEVEDPER');

        /* RESOLVE: Dynamically generated SQL: Dynamically generated SQL, as executed by Execute-Immediate, convert manually */
        /* ORIGSQL: EXECUTE IMMEDIATE v_sql; */
        EXECUTE IMMEDIATE :v_sql;

        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'GAPER AND POINTSACHIEVEDPER for TEAM OVERALL SECTION completed',NULL,'GAPER AND POINTSACHIEVEDPER') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'GAPER AND POINTSACHIEVEDPER for TEAM OVERALL SECTION completed', NULL, 'GAPER AND POINTSACHIEVEDPER');
        --WDAYS For team
        -----Overall Commission-Actual WD/Plan WD..measurement
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin Overall Commission Measurement working day',NULL,'TEAM WDAY') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'Begin Overall Commission Measurement working day', NULL, 'TEAM WDAY');
        v_sql = 'MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using
        (select mes.positionseq,
            mes.payeeseq,
            mes.processingunitseq,
            mes.periodseq,
            MAX(case when rmap.rptcolumnname = ''WDAYSPER'' then '||IFNULL(EXT.FUN_DIRECTSALES_MAPPING(:vrptname, 'WDAYSPER', 'Team', :vPeriodtype),'') ||' end) AS WDAYSPER
            from ext.rpt_BASE_MEASUREMENT mes, ext.rpt_directsales_mapping rmap
        ...
        on (rpt.processingunitseq = qtr.processingunitseq
            ...
            and rpt.products = ''Team''
            and rpt.frequency='''||IFNULL(:vPeriodtype,'')||''')
        when matched then update set rpt.WDAYSPER = (qtr.WDAYSPER*100)';  /* ORIGSQL: fun_directsales_mapping(vrptname,'WDAYSPER','Team',vPeriodtype) */

        /* RESOLVE: Dynamically generated SQL: Dynamically generated SQL, as executed by Execute-Immediate, convert manually */
        /* ORIGSQL: EXECUTE IMMEDIATE v_sql; */
        EXECUTE IMMEDIATE :v_sql;

        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Overall Commission Measurement working day completed',NULL,v_sql) */
        ...
        /* ORIGSQL: UPDATE ext.rpt_INTPREPAIDSNR_INDIVIDUAL SET TOTAL= (nvl(BASICSAMT,0) +nvl(MULTIPLIERAMT,0)),payable_flag=v_payableflag WHERE processingunitseq = vprocessingunitseq AND periodseq = vperiodseq AND allgroups(...) */
        UPDATE ext.rpt_INTPREPAIDSNR_INDIVIDUAL
            SET
            /* ORIGSQL: TOTAL = */
        ...
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'Begin OVERALL SECTION EARNED COMISSION TOTAL',NULL,'Earned Commision') */
        ...
        /* RESOLVE: Identifier not found: Table/view 'STELEXT.RPT_INTPREPAIDSNR_INDIVIDUAL' not found */

        /* ORIGSQL: INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL (tenantid, positionseq, payeeseq, processingunitseq, periodseq, periodname, processingunitname, calendarname, reportcode, sectionid, sectionname,subsec(...) */
        ...
        INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL
            (
                ...
                products, shopname, teamvisible, payable_flag, SECTION_COMMISSION, reportgroup,
                frequency
            )
            SELECT   /* ORIGSQL: SELECT vTenantID,pad.positionseq,pad.payeeseq, :vProcessingUnitRow.processingunitseq, vperiodseq,:vPeriodRow.name,:vProcessingUnitRow.name,:vCalendarRow.name, '59' reportcode,'03' sectionid,'OVERALL COMMI(...) */
                :vTenantId,
                pad.positionseq,
                pad.payeeseq,
            ...
            FROM
                ...
                (
                    SELECT   /* ORIGSQL: (select mes.positionseq, mes.payeeseq, mes.processingunitseq, mes.periodseq, SUM(mes.TOTAL) as SECTION_COMMISSION FROM ext.rpt_INTPREPAIDSNR_INDIVIDUAL mes WHERE processingunitseq = vprocessingunitseq AND(...) */
                        mes.positionseq,
                        mes.payeeseq,
                        mes.processingunitseq,
                        mes.periodseq,
                    ...
                    FROM
                        ext.rpt_INTPREPAIDSNR_INDIVIDUAL mes
                    WHERE
                        processingunitseq = :vprocessingunitseq
                        AND periodseq = :vperiodseq
                        AND allgroups = 'EARNED COMMISSION'
        ...
        /* ORIGSQL: prc_logevent (:vPeriodRow.name,vProcName,'OVERALL SECTION EARNED COMISSION TOTAL Completed',NULL,'Earned Commision') */
        CALL EXT.PRC_LOGEVENT(:vPeriodRow.name, :vProcName, 'OVERALL SECTION EARNED COMISSION TOTAL Completed', NULL, 'Earned Commision');

        FOR i AS dbmtk_cursor_7330
        DO
        ...
        prc_logevent (:vPeriodRow.name,vProcName,'Begin OVERALL COMMISSION Incentive for Individual',NULL,'Individual Payout');
         MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using
        ...
        */
        ...
        INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL
            ...
            FROM
                ...
                ext.rpt_TITLE_PRODUCT_MAPPING titlemap,
                (
                    SELECT   /* ORIGSQL: (select mes.positionseq, mes.payeeseq, mes.processingunitseq, mes.periodseq, SUM(mes.CECOMM) SECTION_COMMISSION from ext.rpt_INTPREPAIDSNR_INDIVIDUAL mes where mes.allgroups='ADJUST COMMISSION' and mes.pe(...) */
                        mes.positionseq,
                        mes.payeeseq,
                        mes.processingunitseq,
                        mes.periodseq,
        ...
        INSERT INTO EXT.RPT_INTPREPAIDSNR_INDIVIDUAL
            ...
            FROM
                ...
                ext.rpt_TITLE_PRODUCT_MAPPING titlemap,
                (
                    SELECT   /* ORIGSQL: (select mes.positionseq, mes.payeeseq, mes.processingunitseq, mes.periodseq, SUM(mes.SECTION_COMMISSION) TOTALCOMMISSION from ext.rpt_INTPREPAIDSNR_INDIVIDUAL mes where mes.allgroups in ('ADJUST REMARKS',(...) */
                        mes.positionseq,
                        mes.payeeseq,
                        mes.processingunitseq,
                        mes.periodseq,
        ...
        /* RESOLVE: MERGE statement: Additional manual conversion of MERGE INTO may be required */
        /* ORIGSQL: MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL rpt using (SELECT distinct mes.positionseq, mes.payeeseq, mes.processingunitseq, mes.periodseq, mes.OTC, mes.GEID, mes.NAME, mes.shopname FROM ext.rpt_INTPREPAIDSNR(...) */
        MERGE INTO ext.rpt_INTPREPAIDSNR_INDIVIDUAL AS rpt 
            USING
            (
                SELECT   /* ORIGSQL: (select distinct mes.positionseq, mes.payeeseq, mes.processingunitseq, mes.periodseq, mes.OTC, mes.GEID, mes.NAME, mes.shopname from ext.rpt_INTPREPAIDSNR_INDIVIDUAL mes where mes.processingunitseq = vpro(...) */
                    DISTINCT
                    mes.positionseq,
                    mes.payeeseq,
                    mes.processingunitseq,
