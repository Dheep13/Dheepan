CREATE OR REPLACE PROCEDURE EXT.TRYG_SP_NOTES_EXTRACT(OUT FILENAME varchar(120), IN vFname varchar(255) default '', pPeriodName varchar(255) default '') --, IN pPlRunSeq BIGINT)
LANGUAGE SQLSCRIPT
...
and pt.name='month'
and inc.genericattribute1 in ('Redemption Grade' , 'Karakter Indløsning')
...
if vFname ='' then
	FILENAME := 'SAPC_MP_' || replace(:v_periodRow.name,' ','_') || '.txt'; --'NOTESEXTRACT.txt';
else
	FILENAME:=vFname;
end if;
