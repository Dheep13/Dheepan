CREATE OR REPLACE PROCEDURE EXT.TRYG_SH_PORTFOLIO ( in_PeriodSeq BIGINT,in_ProcessingUnitSeq BIGINT) 
LANGUAGE SQLSCRIPT 
...
BEGIN
	...
	AND EXISTS (
		...
		/*
		--Incorrect type, only AG+ is valid, as per meeting with Lise skovsted, Anni Grann Jensen, Lisbeth Samuelsen  - AMR 20230615
		*/
		--AND st_in.genericattribute8 IN ('AG+','AG-') -- Relocations
		AND st_in.genericattribute8 IN ('AG+') -- Relocations
		/*
		--Data Mapping Change GD1 to GD6   - AMR 20230615
		*/
		--AND months_between(st_in.genericdate1,st.genericdate1)<=:v_monthDiff
		AND months_between(st_in.genericdate6,st.genericdate6)<=:v_monthDiff
