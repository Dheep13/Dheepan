...
and ta.positionname not like '%OTHER%' --Deepan : TBSV2-1810 fix
and ta.positionname like '%AKF%' --Deepan : TBSV2-1810 fix
...
using 
(select distinct sub.positionname, sub.stagesalestransactionseq, sub.name as titlename, sub.positionname,sub.sublinenumber,sub.alternateordernumber,
c.name as splittype
from cs_category c
...
ti.name, st.stagesalestransactionseq,st.sublinenumber,st.alternateordernumber
from cs_stagesalestransaction st 
inner join cs_stagetransactionassign sta on
sta.stagesalestransactionseq=st.stagesalestransactionseq
inner join cs_position pos on
...
on 
st.stagesalestransactionseq=subq.stagesalestransactionseq
-- st.sublinenumber=subq.sublinenumber
-- and st.alternateordernumber=subq.alternateordernumber
and batchname = :v_batchName
when matched then
    update set st.genericattribute18 = subq.splittype;
commit;

update cs_stagesalestransaction stg set stg.genericattribute18 = (select distinct st.genericattribute18
	from cs_stagesalestransaction st where st.batchname=:v_batchName
	and st.genericattribute18 is not null
	and stg.alternateordernumber=st.alternateordernumber
	and stg.sublinenumber=st.sublinenumber
	and stg.batchname = st.batchname
	and stg.eventtypeid=st.eventtypeid
	and st.eventtypeid=:v_eventType
	and st.genericboolean4=1
	
)
where stg.batchname= :v_batchName
and stg.eventtypeid=:v_eventType;
