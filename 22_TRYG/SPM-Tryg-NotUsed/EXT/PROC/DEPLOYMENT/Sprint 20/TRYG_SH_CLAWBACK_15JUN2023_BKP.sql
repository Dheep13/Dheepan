...
insert into ext.tryg_clawback_credits (select distinct
                            ...
                            FROM cs_salestransaction st_prev,
                               ...
                               and c.genericattribute7 not in ('Afgang','Ikraft nedsæt','Nedsæt')
...
insert into ext.tryg_clawback_credits (
                            ...
                            FROM cs_salestransaction st_prev,
                               ...
                               or(final.isprivateagent=True))
                               and c.compensationdate <= final.cantxns_compdate
                               and(c.genericattribute7 not in ('Afgang','Ikraft nedsæt','Nedsæt') or (final.cantxns_old_premium > final.cantxns_new_premium
                               and final.prev_oldpremium > final.prev_latestpremium)
                               )
                               --and c.compensationdate >= (final.cantxns_policy_sDate)
                               --and c.compensationdate < add_months(final.cantxns_policy_sDate,12)
                               inner join cs_position pos on
...
-- select * from  ext.tryg_clawback_credits;
-- update ta
-- set ta.credit_sum=(select
-- sum(cc.credit_value) as sum_credit_value from ext.tryg_clawback_credits cc
-- where cc.clawbacktype='DECR'
-- and cc.cantxns_compdate <= ta.cantxns_compdate
-- and ta.cantxns_alternateordernumber=cc.cantxns_alternateordernumber
-- )
-- from ext.tryg_clawback_credits ta
-- where exists(select * from ext.tryg_clawback_credits cc
-- 	where cc.cantxns_salestransactionseq=ta.salestransactionseq
-- 	and cc.clawbacktype='DECR'
-- )
-- group by ta.cantxns_alternateordernumber
-- ;

-- update cs_transactionassignment ta
-- inner join ext.tryg_cancel_txns can
-- on ta.salestransactionseq=can.cantxns_salestransactionseq
-- inner join ext.tryg_clawback_credits cc
-- and can.cantxns_alternateordernumber=cc.cantxns_alternateordernumber
-- and cc.cantxns_salestransactionseq=ta.salestransactionseq
-- set ta.genericnumber2=(select
-- sum(cc.credit_value) as sum_credit_value from ext.tryg_clawback_credits cc
-- where cc.clawbacktype='DECR'
-- and ta.compensationdate > cc.cantxns_compdate)
-- where exists(select * from ext.tryg_clawback_credits cc
-- 	where cc.cantxns_salestransactionseq=ta.salestransactionseq
-- 	and cc.clawbacktype='DECR'
-- );
...
merge into cs_transactionassignment ta
using
(select cantxns_salestransactionseq,credit_value, prev_latestpremium, prev_positionname from ext.tryg_clawback_credits clc
where clc.clawbacktype='GENI'
) sub
on ta.salestransactionseq = sub.cantxns_salestransactionseq
and ta.positionname = sub.prev_positionname
