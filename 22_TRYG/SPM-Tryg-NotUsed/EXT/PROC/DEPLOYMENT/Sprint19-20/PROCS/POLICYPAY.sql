...
/*---------------------------------------------------------------------
    ...
    | Version: 0.1	01-May-2022	Intial Version
      Version: 0.2	22-May-2022	Intial Version
    -----------------------------------------------------------------------
    */
BEGIN
...
---Reset the policy paid flag for summary transctions
UPDATE 
st
...
and ifnull(cc.creditvalue,9999999999) <> 9999999999;
    ...
    --Reset payment flags for SC-DK-001-002
    UPDATE
		cs_transactionassignment ta SET (ta.genericnumber1,ta.unittypeforgenericnumber1,ta.genericboolean1) =(0,:v_ctDKKRow.unittypeseq,0)
	    where exists (
...
UPDATE 
st
set st.genericboolean5=1, genericattribute21 = cc.alternateordernumber || '-' || cc.sublinenumber || '-' || cc.linenumber
from cs_salestransaction st
inner join ext.TRYG_SH_SALESTXNS_POLICYCRDS cc on
st.alternateordernumber = cc.alternateordernumber
inner join cs_transactionassignment ta
...
AND genericattribute21 IS NULL;
  ...
  CALL EXT.TRYG_LOG(v_procedureName,'Set summary transactions paid flag (gb5)',::ROWCOUNT);
	UPDATE
