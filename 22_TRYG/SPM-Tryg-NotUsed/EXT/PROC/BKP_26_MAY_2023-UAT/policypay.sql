...
BEGIN
	...
	*/
    ...
    v_sqlCount = ::ROWCOUNT;
	CALL EXT.TRYG_LOG(v_procedureName,'Delete duplicate records for positions',v_sqlCount);
	COMMIT;
...
/*
	...
	SET
		...
		(
			...
			join cs_period per1 on per1.periodseq=cc.periodseq
			WHERE
				...
				AND cc.positionseq in
					...
					and per1.startdate<:v_periodRow.startdate
...
*/
	...
	Truncate table ext.TRYG_POLICY_PAY_CREDITS_TEMP;
	
	insert into ext.TRYG_POLICY_PAY_CREDITS_TEMP
	(
			SELECT
				sum(IFNULL(cc.value,0)),
				1970324836974600,
				st.alternateordernumber,--cc.unittypeforvalue
				pc.positionseq
			FROM
				cs_credit cc
			JOIN cs_salestransaction st ON st.salestransactionseq = cc.salestransactionseq
			JOIN cs_eventtype et ON et.datatypeseq = st.eventtypeseq
			join cs_period per1 on per1.periodseq=cc.periodseq
			join ext.TRYG_SH_SALESTXNS_POLICYCRDS pc on st.alternateordernumber = pc.alternateordernumber
			WHERE
				st.compensationdate <  pc.compensationdate
				AND st.compensationdate >= pc.LASTPAYMENTDATE
				AND et.removedate = v_removedate
				...
				AND cc.positionseq in
					(	
						...
						and per.periodseq = :v_periodRow.periodseq --cc.periodseq
						--and poscr.ruleelementownerseq=4785074604084466 --4785074604083402
					) -- = pc.positionseq
					and per1.startdate<:v_periodRow.startdate  --cc.periodseq<2533274790396049
					and cc.value is not null
			GROUP BY 
				1970324836974600, st.alternateordernumber, pc.positionseq --cc.unittypeforvalue
);
	...
	SET
		(creditvalue,unittypeforcreditvalue) = 
		(
			select creditvalue, unittypeforvalue
			from ext.TRYG_POLICY_PAY_CREDITS_TEMP ppc
			where ppc.alternateordernumber=pc.alternateordernumber
			and ppc.positionseq=pc.positionseq
		)
	where pc.periodseq=:v_periodRow.periodseq;
