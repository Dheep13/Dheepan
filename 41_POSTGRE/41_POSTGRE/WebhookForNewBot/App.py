...
@app.route('/GetPeriodSeq',methods=['POST','GET'])
def GetPeriodSeq():
    ...
    ##########      GET SLUG        ##############
        ...
        periodname=botdata['conversation']['memory']['name']
        print('period name is ' + str(periodname))
                
        # base_url = api_url+"periods?$filter=(name eq "+periodname+")"
        base_url = api_url + "periods?$filter=(name eq '" + periodname + "')"
        print(base_url)
        ...
        print(periods)
        periodSequence=periods['periods'][0]['periodSeq']

        print(periodSequence)

    return jsonify(
        status=200,
        replies=[{
        'type': 'text',
        'content': '%s' %(periodSequence)
        }]
        ) 
...
def AllInvoices():
    ...
    ##########      GET SLUG        ##############
        ...
        skill = botdata['conversation']['skill']
        positionseq=botdata['conversation']['memory']['positionseq']
        orderid = botdata['conversation']['memory']['orderid']
        print('positionseq is ' + str(positionseq))
        print('salesorderseq is ' + str(orderid))
        
        get_salesorderseq = api_url+"salesOrders?$filter=(orderId eq "+str(orderid) +" )"
        print(get_salesorderseq)
        s = requests.get(get_salesorderseq, auth=(user,key))
        # print('this is the output' + str(r.text))
        salesorderseq_data = json.loads(s.text)
        salesorderseq=salesorderseq_data['salesOrders'][0]['salesOrderSeq']

        base_url = api_url+"salesTransactions?$filter=(salesOrder eq "+str(salesorderseq) +" and genericAttribute1 ne null)"
        print(base_url)
        r = requests.get(base_url, auth=(user,key))
        # print('this is the output' + str(r.text))
        invoice_data = json.loads(r.text)
        ...
        for x in invoices:
            if x is not None:
                ...
                invoices_str += ""  # Or simply skip or use "" for an empty string
            invoices_str += '\n'
...
def MissingInvoice():
    ...
    if (request.method == 'POST'):
        ...
        missingInvoice=botdata['conversation']['memory']['missingInvoice']
