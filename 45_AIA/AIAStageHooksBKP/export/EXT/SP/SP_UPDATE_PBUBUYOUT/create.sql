...
DECLARE EXIT HANDLER FOR SQLEXCEPTION
        BEGIN
            ...
            ROLLBACK;
            resignal; --Deepan : added resignal since the dml on cs table was not erroring
        END;
...
SET SESSION 'ISMONTHEND' = CAST(:gv_isMonthEnd AS VARCHAR(512));
    ...
    if gv_isMonthEnd = 1 then
        ...
        select    ed5,
            ...
            Log('Inside loop for v_PromoteeCount is greater than 0');
...
--Deepan : Comment out v_sql logic if not required
     v_sql =   'Update Cs_Credit t
 ...
 --Deepan : UnComment the below statement 
  /*      execute immediate 'Update Cs_Credit t
    ...
    */     
        log('Number of Rows updated in CS_CREDIT TABLE :'|| ::ROWCOUNT) ;
        commit;
        Log('14');
...
*/
       ...
       log('Number of Rows updated in CS_MEASUREMENT TABLE :'|| ::ROWCOUNT) ;
        ...
        Log('17');
            ...
            Log('v_PromoteeCount is 0');
        end if;    
        
        else
        Log('12 - Not month end');
        Log('gv_isMonthEnd is False');
