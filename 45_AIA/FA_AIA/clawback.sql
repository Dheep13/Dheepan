procedure SP_CLAWBACK_CALCULATION(i_periodSeq in int) as
...
if to_date(v_str_cycledate,'yyyy-mm-dd') between v_period_start and v_period_end then
    ...
    Log('[CB] SP_EXEC_COMMISSION_ONGOING starting '|| v_str_cycledate);
    PK_AIA_CB_CALCULATION.SP_EXEC_COMMISSION_ONGOING(v_str_cycledate);
    Log('[CB] SP_EXEC_COMMISSION_ONGOING ending');

    Log('[CB] SP_EXEC_COMPENSATION_ONGOING starting');
    PK_AIA_CB_CALCULATION.SP_EXEC_COMPENSATION_ONGOING(v_str_cycledate);
    Log('[CB] SP_EXEC_COMPENSATION_ONGOING ended');
...
;
   ...
   select nvl(max(batchnum), 0)
    ...
    PK_AIA_CB_CALCULATION.sp_revert_by_batch(v_batch_no_lumpsum_comm);
    ...
    PK_AIA_CB_CALCULATION.sp_revert_by_batch(v_batch_no_ongoing_comm);
    ...
    PK_AIA_CB_CALCULATION.sp_revert_by_batch(v_batch_no_lumpsum_comp);
    ...
    PK_AIA_CB_CALCULATION.sp_revert_by_batch(v_batch_no_ongoing_comp);


    --prepare the commission clawback data for insert to TrueComp build in tables
    --credit
    PK_AIA_CB_CALCULATION.SP_CREDIT_COMMISSION (v_str_cycledate, v_batch_no_lumpsum_comm, v_batch_no_ongoing_comm);
    --primary measurement
    PK_AIA_CB_CALCULATION.SP_PM_COMMISSION (v_str_cycledate, v_batch_no_lumpsum_comm, v_batch_no_ongoing_comm);
    --pm credit trace
    PK_AIA_CB_CALCULATION.SP_PMCRDTRACE_COMMISSION (v_str_cycledate, v_batch_no_lumpsum_comm, v_batch_no_ongoing_comm);
    ...
    --credit
    PK_AIA_CB_CALCULATION.SP_CREDIT_COMP (v_str_cycledate, v_batch_no_lumpsum_comp, v_batch_no_ongoing_comp);
    --primary measurement
    PK_AIA_CB_CALCULATION.SP_PM_COMP (v_str_cycledate, v_batch_no_lumpsum_comp, v_batch_no_ongoing_comp);
    --pm credit trace
    PK_AIA_CB_CALCULATION.SP_PMCRDTRACE_COMP (v_str_cycledate, v_batch_no_lumpsum_comp, v_batch_no_ongoing_comp);
...
Log('[CB] Get previous quarter for SPI CB' || '; row count: ' || to_char(sql%rowcount));
...
Log('[CB] Merge records into cs_measurement for SPI ONGOING YTD CB' || '; row count: ' || to_char(sql%rowcount));
--Version 12 end

--update the batch status to complete
PK_AIA_CB_CALCULATION.sp_update_batch_status(v_batch_no_lumpsum_comm, STR_STATUS_COMPLETED_SH);
PK_AIA_CB_CALCULATION.sp_update_batch_status(v_batch_no_ongoing_comm, STR_STATUS_COMPLETED_SH);
