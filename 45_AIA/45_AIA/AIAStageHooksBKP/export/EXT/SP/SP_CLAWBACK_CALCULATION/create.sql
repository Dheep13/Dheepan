CREATE PROCEDURE EXT.SP_CLAWBACK_CALCULATION(IN i_periodSeq BIGINT)
LANGUAGE SQLSCRIPT
AS
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        ...
        BEGIN
            ...
            CALL  ext.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_lumpsum_comm, :STR_STATUS_FAIL);

            /* ORIGSQL: PK_AIA_CB_CALCULATION:sp_update_batch_status(v_batch_no_ongoing_comm, STR_STATUS_FAIL) */
            CALL  EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_ongoing_comm, :STR_STATUS_FAIL);

            /* ORIGSQL: PK_AIA_CB_CALCULATION:sp_update_batch_status(v_batch_no_lumpsum_comp, STR_STATUS_FAIL) */
            CALL  EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_lumpsum_comp, :STR_STATUS_FAIL);

            /* ORIGSQL: PK_AIA_CB_CALCULATION:sp_update_batch_status(v_batch_no_ongoing_comp, STR_STATUS_FAIL) */
            CALL  EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_ongoing_comp, :STR_STATUS_FAIL);
   ...
   select tenantid into v_tenantId from cs_tenant;
    ...
    IF TO_DATE(:v_str_cycledate, 'yyyy-mm-dd') BETWEEN :v_period_start AND :v_period_end THEN
        ...
        CALL EXT.PK_AIA_CB_CALCULATION:SP_EXEC_COMMISSION_ONGOING(:v_str_cycledate);
        ...
        CALL  EXT.PK_AIA_CB_CALCULATION:SP_EXEC_COMPENSATION_ONGOING(:v_str_cycledate);
        ...
        v_batch_no_lumpsum_comm :=  EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMMISSION, :STR_LUMPSUM, :STR_STATUS_COMPLETED_SP);

        IF :v_batch_no_lumpsum_comm = 0 THEN
            v_batch_no_lumpsum_comm :=  EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMMISSION, :STR_LUMPSUM, :STR_STATUS_COMPLETED_SH);
        END IF;

        CALL EXT.Log('[CB] Clawback batch for commission LUMPSUM: ' || :v_batch_no_lumpsum_comm);
        CALL EXT.PK_AIA_CB_CALCULATION:sp_revert_by_batch(:v_batch_no_lumpsum_comm);

        -- Get active batch number for on-going commission
        v_batch_no_ongoing_comm :=  EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMMISSION, :STR_ONGOING, :STR_STATUS_COMPLETED_SP);
        CALL EXT.Log('[CB] Clawback batch for commission ONGOING: ' || :v_batch_no_ongoing_comm);
        CALL EXT.PK_AIA_CB_CALCULATION:sp_revert_by_batch(:v_batch_no_ongoing_comm);

        -- Get active batch number for lumpsum compensation
        v_batch_no_lumpsum_comp :=  EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMPENSATION, :STR_LUMPSUM, :STR_STATUS_COMPLETED_SP);
        IF :v_batch_no_lumpsum_comp = 0 THEN
            v_batch_no_lumpsum_comp :=  EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMPENSATION, :STR_LUMPSUM, :STR_STATUS_COMPLETED_SH);
        END IF;
        CALL EXT.Log('[CB] Clawback batch for compensation LUMPSUM: ' || :v_batch_no_lumpsum_comp);
        CALL EXT.PK_AIA_CB_CALCULATION:sp_revert_by_batch(:v_batch_no_lumpsum_comp);

        -- Get active batch number for on-going compensation
        v_batch_no_ongoing_comp := EXT.PK_AIA_CB_CALCULATION:fn_get_batch_no(:v_str_cycledate, :STR_COMPENSATION, :STR_ONGOING, :STR_STATUS_COMPLETED_SP);
        CALL EXT.Log('[CB] Clawback batch for compensation ONGOING: ' || :v_batch_no_ongoing_comp);
        CALL EXT.PK_AIA_CB_CALCULATION:sp_revert_by_batch(:v_batch_no_ongoing_comp);

        -- Prepare the commission clawback data for insert to TrueComp build in tables
        -- Credit
        CALL EXT.PK_AIA_CB_CALCULATION:SP_CREDIT_COMMISSION(:v_str_cycledate, :v_batch_no_lumpsum_comm, :v_batch_no_ongoing_comm);
        -- Primary measurement
        CALL EXT.PK_AIA_CB_CALCULATION:SP_PM_COMMISSION(:v_str_cycledate, :v_batch_no_lumpsum_comm, :v_batch_no_ongoing_comm);
        -- PM credit trace
        CALL EXT.PK_AIA_CB_CALCULATION:SP_PMCRDTRACE_COMMISSION(:v_str_cycledate, :v_batch_no_lumpsum_comm, :v_batch_no_ongoing_comm);

        -- Prepare the compensation clawback data for insert to TrueComp build in tables
        -- Credit
        CALL EXT.PK_AIA_CB_CALCULATION:SP_CREDIT_COMP(:v_str_cycledate, :v_batch_no_lumpsum_comp, :v_batch_no_ongoing_comp);
        -- Primary measurement
        CALL EXT.PK_AIA_CB_CALCULATION:SP_PM_COMP(:v_str_cycledate, :v_batch_no_lumpsum_comp, :v_batch_no_ongoing_comp);
        -- PM credit trace
        CALL EXT.PK_AIA_CB_CALCULATION:SP_PMCRDTRACE_COMP(:v_str_cycledate, :v_batch_no_lumpsum_comp, :v_batch_no_ongoing_comp);
...
v_sql := 'MERGE INTO cs_measurement AS tgt USING (' ||
         ...
         'WHEN MATCHED THEN UPDATE SET tgt.planseq = src.planseq, tgt.ruleseq = src.ruleseq, tgt.value = src.value, ' ||
...
CALL EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_lumpsum_comm, :STR_STATUS_COMPLETED_SH);
CALL EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_ongoing_comm, :STR_STATUS_COMPLETED_SH);

    --Added by Win Tan for version 4 **Begin**
    if to_date(:v_str_cycledate,'yyyy-mm-dd') = :v_period_end then
    CALL EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_lumpsum_comp, :STR_STATUS_COMPLETED_SH);
    CALL EXT.PK_AIA_CB_CALCULATION:sp_update_batch_status(:v_batch_no_ongoing_comp, :STR_STATUS_COMPLETED_SH);
    end if;
...
CALL EXT.LOG('v_str_cycledate: '|| TO_DATE(:v_str_cycledate, 'yyyy-mm-dd')||' v_period_start: '|| :v_period_start ||' v_period_end: ' ||:v_period_end);
...
*/--Deepan : Handles at the top of the proc in exit handler
end