CREATE LIBRARY "EXT"."PK_AIA_CB_CALCULATION" LANGUAGE SQLSCRIPT AS
BEGIN
  ...
  PUBLIC VARIABLE STR_BUNAME CONSTANT VARCHAR(20) = 'SGPAGY';
...
BEGIN
    ...
    DECLARE V_Curr_QTR NVARCHAR(30);  /* ORIGSQL: V_Curr_QTR VARCHAR2(30); */
    DECLARE V_Previous_QTR NVARCHAR(30);  /* ORIGSQL: V_Previous_QTR VARCHAR2(30); */
    DECLARE V_Second_QTR NVARCHAR(30);
    DECLARE V_First_QTR  NVARCHAR(30);  /* ORIGSQL: V_First_QTR INTEGER ; */
...
BEGIN
    ...
    */
    ...
    THEN
        -- vCreditOffset = TCMP.SequenceGenPkg.GetNextFullSeq('creditSeq', 56, :V_REC_COUNT) - 1; 
        -- vCreditOffset = 100; ---Deepan : hard coded to 100 as a temporary workaround 
        v_sql := 'CALL TCMP.SEQUENCEGENLIB:GetNextSeq(''creditSeq'', ?, ?)';
		EXECUTE IMMEDIATE :v_sql INTO vCreditOffset USING  (:V_REC_COUNT-1);
        /* ORIGSQL: update EXT.AIA_CB_CREDIT_STG SET new_creditSeq = new_creditSeq + vCreditOffset where batch_no in (P_BATCH_NO_1, P_BATCH_NO_2); */
        UPDATE EXT.AIA_CB_CREDIT_STG
...
BEGIN
    ...
    THEN
        -- vPMOffset = TCMP.SequenceGenPkg.GetNextFullSeq('measurementSeq', 64, :V_REC_COUNT) - 1; 
		-- vPMOffset = 100; --Deepan : Hardcoded to 100 as a temporary workaround
		v_sql := 'CALL TCMP.SEQUENCEGENLIB:GetNextSeq(''measurementSeq'', ?, ?)';
		EXECUTE IMMEDIATE :v_sql INTO vPMOffset USING (:V_REC_COUNT-1);
