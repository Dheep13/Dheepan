...
def split_large_xml(xml_file, output_folder):
    ...
    for index, mapping in enumerate(root.findall('.//MAPPING'), start=1):
        ...
        output_path = os.path.join(output_folder, f'mapping_{index}.xml')
...
def process_xml_to_excel(xml_file):
    ...
    mapping_name = root.attrib.get('NAME') if root is not None else "No Mapping Name Found"
...
def main():
    ...
    # Assume large XML is directly extracted here
